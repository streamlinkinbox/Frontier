...
int main(){
  ...
  std::printf("\n%-10s %6s %5s %6s %6s %5s %5s %5s %5s %6s %6s\n","tier","cloud","tap","local","cldRes","atmN","atmL","starL","starA","godray","covMgn");
  for(int i=0;i<5;i++) std::printf("%-10s %6u %5u %6u %6.2f %5u %5u %5u %5u %6u %6.2f\n",N[i],
    ...
    K[i].GodRaySampleCount,(double)K[i].CloudCoverageMargin);
  int bad=0;
  for(int i=1;i<5;i++){
    ...
    if(K[i].GodRaySampleCount     < K[i-1].GodRaySampleCount)     {std::printf("  FAIL god rays drop at %s\n",N[i]);bad=1;}
  ...
  if(K[0].GodRaySampleCount!=0){std::printf("  FAIL Minimal should disable god rays\n");bad=1;}
