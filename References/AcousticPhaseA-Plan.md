...
### 2b. Rev 3 voice (LaFerrari first — `voice.crank_pulse = true`; the 918 / GT-R stay on rev 2, bit-identical)

After the voicing report (`AcousticPhaseA-VoicingReport.md` §5 a–i, §10.4 measured idle targets) you asked for the LaFerrari to be
improved in the editor before the C++ port. Rev 3 is a second voice inside the same `AcousticIntegrator`, switched per car by
`voice.crank_pulse`; with it off the DSP is the rev-2 path unchanged (`/tmp` `cmp.js`: 3 cars × 6 renders against the previous
commit, max |Δ| exactly 0 — the 918 and GT-R sound as before and the A2 port plan still holds for them).

| §5 entry | Rev 3 (`AcousticIntegrator`, JS; A2 ports it) | Measured (LaFerrari, left channel, full chain) |
|---|---|---|
| a bank-true routing | `bank_of_cylinder` = Ferrari 1–6 / 7–12; `pan_split_hz` (600) = LR2 crossover per bank: the low band of both banks is one image, the high band is panned by `bank_pan` (low − high sums to an allpass, so pan 0 leaves the spectrum flat) | structural half-orders (variance off) ≤ −70 dB re order 6; with the cycle variance on the between-order floor is −42 dB mean / −28 worst (real loops −36 … −42 / −22) |
| b crank-true angles | `even_firing = false` + `bank_angle_deg = 65`: each bank fires at 120°, bank 1 trails by 65° → 55° / 65° alternation from the crank clock | orders 7 / 8 present at −39 / −23 (real −4 … −19) |
| c pulse in crank degrees | one kernel per firing over 300° after EVO (raised-cosine blowdown rise `pulse_rise_deg`, exponential decay `pulse_blowdown_deg`, displacement half-sine × `pulse_tail_level` through the valve's open-area window, suck-back after TDC × `pulse_undershoot`); the kernel is the valve discharge DIFFERENTIATED (open-pipe radiation p ∝ dU/dt, zero-mean); hard ↔ soft kernels crossfaded by load (`pulse_soft_level`, `pulse_soft_stretch`) | order 3 at idle −27 dB re 6 (rev 2 −16 … −18; real −35); the displacement half-sine has no odd harmonics, which is where the real idle's missing order 3 comes from |
| d variance = pressure | `jitter_amp` (per-cycle amplitude COV, × (1 + `idle_jitter_boost`) toward idle), `cylinder_spread_pct` = fixed once-per-revolution imbalance pattern along the firing order (zero-mean, no order-3 part → integer orders 1 / 2 / 4 / 5 / 7 / 8 as in the real loops, no half-orders), `timing_jitter_deg` ≤ 0.3° non-cumulative, `jitter_pct` walk 0 | order 6 stays a line; orders 1 / 2 / 4 / 8 at −15 / −11 / −20 / −23 (real rev loop −27 / −13 / −24 / −15) |
| e intake voice | second event train at the TDC overlap (`INTAKE_EVENT_DEG`): inverted suction kernel through the runner resonator (f, 3f; `runner_hz` 400, `q` 3) + the trumpet mouth's direct high-passed radiation (`direct`), tract steps at `step_rpm_1` 6000 (× `short_ratio` 2) and `step_rpm_2` 8000 (× `min_ratio` 2.6), breath noise, level ∝ throttle · rpm / redline, and `path_m` 1.5 (a delay so the two crank-locked voices interfere like two separated emitters instead of cancelling one order over the whole rev range); rides the induction bus (cockpit 1.2 ×, trackside 1 ×) | intake bus −10 … −14 dBFS above 6000 rpm at WOT vs exhaust −11; centroid 850 Hz at 4500 → 1600 Hz at 9000 rpm |
| f exhaust formants | `formant_a/b` 180 / 215 Hz (Q 8, 0.3 / 0.25) after the tanh, `comb_mix 0` (the 2.667 ms comb is out), 4th-order silencer low-pass `silencer_hz` 170 with the valve shut | idle centroid 129 Hz, nothing within 37 dB above 300 Hz, orders 18 / 24 at −38 / −58 (real −40 / −56), 6 lines within 26 dB below 1 kHz (real 10) |
| g valve step | opens above `valve_open_rpm` 3500 **or** `valve_open_load` 0.55: `valve_open_gain` 1.4, `silencer_open_hz` 4000, 0.2 s lag | +3 dB and the high band step at 3500 rpm on the pull |
| h overrun | kept sparse (`backfire_probability` 0.05, `pop_rate_hz` 6) | 4–6 pops per lift on the dyno sequences |
| i idle mechanics | `clatter` 0.02 at 4800 Hz (valvetrain hash), crackle only under load (`crackle_level` 0, `crackle_load_level` 0.15) | 2–6 kHz ≥ 29 dB below the 20–200 Hz band at idle (real ≥ 25) |
| level | `mix.output_gain` 0.55, `exhaust.drive` 0.8: the ±1 clip stays idle on every dyno sequence trackside and cockpit (rev 2 leans on it, 11–36 % of samples) | pull / overrun / limiter peaks 0.90–0.92, 0 clipped samples |

Not done (needs the §8 recordings): the formant frequencies above 300 Hz, the exact valve rpm, the pop count, and the whole
loaded range above idle — the only real LaFerrari audio reachable covers the idle alone (report §10.5). Schema now **111 fields in
8 sections** (`vehicle 15, combustion 12, voice 29, intake 10, exhaust 19, mechanical 7, turbo 14, mix 5`); the three archives
are regenerated from the schema (918 / GT-R: new fields at their rev-2-equivalent defaults) and stay byte-identical to the
embedded blocks.
...
### 3a. Two more vehicles (row A2½ — you asked for the Agera R and the Demon from the RevSim list, on the rev-2 voice)

| | Koenigsegg Agera R | Dodge Challenger SRT Demon (2018) |
|---|---|---|
| Engine | 5,032 cc 90° V8, **cross-plane crank**, twin turbo 1.4 bar, dry sump, inconel merge-collector exhaust | 6,166 cc 90° **cross-plane** HEMI V8, **IHI 2.7 L twin-screw supercharger** (14.5 psi ≈ 1.0 bar), integrated bypass valve |
| Bore × stroke | 91.7 × 95.25 mm | 103.9 × 90.9 mm |
| Peak / redline | 1,140 hp @ 7,100 · **7,500 rpm** · 1,200 N·m @ 4,100 | 840 hp @ 6,300 · **6,500 rpm** · 1,044 N·m @ 4,500 |
| Firing | cross-plane 90° V8, Ford-modular order 1-3-7-2-6-5-4-8 (cyl 1–4 = one bank) ⇒ the L-R-L-L-R-L-R-R bank pattern: 2nd / 4th / 6th orders per bank, the half-order burble a cross-plane carries | classic Chrysler 1-8-4-3-6-5-7-2 (odd = left bank), the same cross-plane bank pattern, longer stroke, lower idle (800) |
| Forced induction | `[turbo]`: whine 4 → 12 kHz with the spool (RevSim "loud turbos" 0.7), rush 2 → 8 kHz, **wastegate chatter** on lift: blow-off with a **25 Hz flutter + a 60 Hz thump** (`turbo.flutter_hz / thump_level / thump_hz`, new) | `[charger]` (new section): rotor pulsation at lobes × drive ratio = 3 × 2.36 = **order 7.08** re crank (472 Hz @ 4,000 rpm, 767 Hz @ 6,500) with its harmonics through a tanh, level ∝ rpm / redline · load, `bypass_level` 0.15 with the throttle shut, boost = 1.0 bar × rpm / redline × load (no spool), **intake bark** on a fast stab (bypass valve snapping shut) |
| Dominant order at peak | 4th: 473 Hz @ 7,100 | 4th: 420 Hz @ 6,300 — the blower's 7.08 sits at −7 dB re the loudest line at 6,000 rpm WOT, −30 dB shut |
| Idle | ≈ 900 rpm | ≈ 800 rpm |
| RevSim vs the real car (⚠️) | RevSim's `agera` is flat-plane 1-5-4-8-2-6-3-7 at 8,250 — that is the Agera **RS / Jesko** direction; Koenigsegg's own page calls the Jesko's 180° crank "Koenigsegg's first flat-plane crankshaft", so the Agera R is built **cross-plane** here | RevSim's supercharger is three sines at rpm × 1.5 × {1, 2, 3}; here it is the rotor lobe-pass order of the actual IHI (3-lobe male / 5-lobe female, 2.36 : 1) so the whine climbs with the crank at the pitch a Hellcat / Demon recording shows |
...
| **A1¾** LaFerrari rev 3 in the editor ✅ (you asked for the LaFerrari to be improved in the editor before the port; report §5 / §10.4) | `voice.crank_pulse` voice in the shared DSP (§2b): crank-true 55/65° firings, crank-degree blowdown / displacement kernels (differentiated valve discharge), hard ↔ soft by load, pressure variance + revolution-locked imbalance pattern, LR2 bank pan split, intake voice with the 6000 / 8000 tract steps + mouth radiation + path delay, exhaust formants + 4th-order silencer, valve by rpm or load, headroom (no clipping); embedded LaFerrari TOML + `EngineContent/AudioArchives/FerrariLaFerrari/FerrariLaFerrari.toml` retuned (`Scratchpad/AcousticLaFerrariRev3Overrides.json` = the delta over rev 2); signal chain gains the intake junction (meter 11), the readout shows the kernel span + valve position | `Scratchpad/AcousticEditorRender.log` **62/62**: the 48 rev-2 checks (918 / GT-R renders bit-identical to the previous commit — `max |Δ| 0` over 18 renders) + the §10.4 idle sheet per channel (loudest order 6; structural half-orders ≤ −40 → −70; between-order floor −42 mean; order 3 −27; orders 18 / 24 −38 / −58; centroid 129 Hz; > 300 Hz −37 dB; 6 lines) + no clipped samples on pull / overrun / limiter, trackside and cockpit; `Scratchpad/AcousticEditor_FerrariLaFerrari_Pull.png` | `Tools\AudioEditor\index.html` → Start audio → key **2** (LaFerrari) → hold Space, `pull`, `overrun`, `blip`, listener trackside ↔ cockpit; bar: "a V12 Ferrari at idle and on the pull" — say what is still wrong (the top end above 6000 is tuned blind, no recording) |
| **A2** synthesis port ✅ (rows P0–P5 of `AcousticPhaseA-CppPortPlan.md`, one commit) | C++ port of the shared DSP **as it stands after A1¾ — rev 2 and rev 3 in one loop**, 1:1: `Engine/PlatformInterchange/{PowertrainRecord.h, SignalSections.h, TransientSlots.h, AcousticStructure.h/.cpp, AcousticIntegrator.h/.cpp}` (111-field sheet generated from `ACOUSTIC_SCHEMA`, tomlpp load / byte-identical save, crank clock + shared-pin angles, kernels, sheets, intake voice, LR2 pan split, formants / silencer / valve, transients, turbo, mechanical, meters, scope, structure relay); `Projects/Project-Dyno`: `--car / --listener / --pure / --seed / --free / --slice / --save`, `DynoSequence` + `overrun` + `limiter` + `ScriptedRecord` load rule, `FreeRevPowertrain`, `[Audio]` line with the voice's counters, `--clicks` keeps the A1 click train; editor: C++ WAV order overlay (Split / Δ) + agreement readout | `Scratchpad/AcousticIdentityTest.log` **70/70**: 14 JavaScript reference dumps (all 7 LaFerrari pulls, 918 pull / blip, GT-R pull / overrun, pure, cockpit, 37-frame slices) vs the C++ render **max \|Δ\| 1.7 × 10⁻¹¹** per sample (bar 10⁻⁶), every counter and the final crank angle equal; archives round-trip byte for byte; 1 / 37 / 64 / 256 slices identical; 34 µs per 64-frame slice (2.5 % of the budget). `Scratchpad/ProjectDynoRender.log`: `--render` WAV vs dump ≤ 3 × 10⁻⁸ (float32 rounding) for 3 cars, order sheets + `Scratchpad/ProjectDyno_<Car>_Pull.png`, pure steady: firing order within 20 dB of the loudest, 60 s null device 0 overloads / 0 dropped, peak callback 440 µs. `Scratchpad/AcousticEditorBrowser.log` **34/34**: C++ WAV order sheet vs the worklet **0.0000 dB**, Export TOML == `--save` byte for byte, `Diagnostics/AudioEditor_07_CppOverlay.png` | `ToolchainSequence.ps1 -Run -RunArguments '--car','FerrariLaFerrari','--pull','pull'` — the same LaFerrari pull as the editor's; `--free` + Space revs like the editor; `[Audio]` peak µs < 500, overloads 0; drop a `--render` WAV on the editor → Split / Δ shows the two order diagrams coincide |
| **A2½** Agera R + Demon + vehicle dropdown ✅ (your request after A2: "in html we would also want the Koenigsegg Agera R; +Dodge Demon; the Audio sim has it … make the Audio choosing for vehicle a dropdown") | Two new archives `EngineContent/AudioArchives/{KoenigseggAgeraR,DodgeDemon}/` (§3a) embedded in the editor; the shared DSP grows a `[charger]` section (positive-displacement supercharger: lobe-pass pulsation, bypass, bark, boost) and three `[turbo]` fields (`flutter_hz`, `thump_level`, `thump_hz`) — **124 fields / 9 sections**; the C++ twin (`TransientSlots.h` Thump / Bark categories + flutter argument, `AcousticStructure` ChargerSheet + regenerated `Fields` from the new `Scratchpad/AcousticSheetGenerate.js --check`, `AcousticIntegrator` step 6b, meter 10 = supercharger); the three older archives re-serialised with the new fields at their defaults (**bit-neutral**: all 14 pre-existing reference dumps identical before / after the DSP change); the top-strip car buttons become the `References/UIComponents.html` `.dd` dropdown (name · engine line · key badge; keys 1 … 9 pick by position, Escape closes, outside click closes); chain junction reads *Turbo* or *Supercharger* per car; boost readout follows either | `AcousticEditorRender.js` 94/94 (5 cars); `AcousticEditorBrowser.js` 45/45 (dropdown open / pick / outside-click / keys, Demon charger meter + boost, Agera boost + lift transients, worklet ⇄ node 2.98e-8, C++ overlay 0.0000 dB, Export == `--save`); `CheckAcousticIdentity.sh` 90/90 — 18 dumps, worst |Δ| 1.72e-11; `CheckAcousticDyno.sh` 5 cars ≤ 2.98e-8, null run 0 overloads; `CheckAcousticArchives.sh` 5 × 124 fields | dropdown or keys **4** / **5** → Agera R / SRT Demon; Space; `pull` on each; `overrun` on the Agera (wastegate chatter + whump on lift); a stab from idle on the Demon (bark); `ToolchainSequence.ps1 -Run -RunArguments '--car','KoenigseggAgeraR','--pull','pull'` and `'--car','DodgeDemon'` |
...
| **A4** editor live ✅ | ~~worklet port, scopes, order diagram, TOML I/O, file feed, WAV record~~ done in A1½; ~~C++ ⇄ worklet identity, `--render` order overlay~~ done in A2 (Split / Δ on the order diagram); **reference lane** as built: a third feed *Reference* (drop any decodable recording — WAV / MP3 / M4A / OGG / FLAC — or Shift-drop / feed button); the recording's rpm is tracked blind from the audio in a Worker (`<script id="analysis">`: quarter-order lattice + **template matching against the selected car's own order bank** — ≈ 65 steady synth renders per car on a 4 % rpm grid at WOT and closed throttle, built once per session in 2–6 s — Viterbi over 0.5 % rpm rungs, confidence from the rival margin, a per-segment octave vote; frames below confidence are drawn dim and skipped); the synth plays **along that trace** (`DynoSequence.assignKeys` — JS sequencing only, no C++ mirror; the `#dsp` text change is bit-neutral) with a throttle heuristic (rising rpm → open); hear Recording / Synth / Both; ÷2 / ×2 fix an octave slip; click the trace to seek; the order diagram's Split / Δ compares recording (upper) against synth (lower) along the same trace, both read as the mono mix, level-aligned, with the mean \|Δ\| as the agreement line; Compare re-renders after inspector edits (auto after 1.2 s). ⚠️ no real recordings exist in the tree — the tracker is validated on the page's own renders (other seed, 16-bit, plus pink noise / room / telephone degradations) and a wrong car pick puts the trace an octave out by design (the note names the car); renders capped at 180 s | `Scratchpad/AcousticReferenceLane.js` 23/23 — 12 blind cases worst median 0.59 %, 0 octave slips, coverage ≥ 71 %; degraded ≤ 0.75 %, 0 slips; lane agreement 1.92 dB mean over 2338 cells (floor along the true rpm 1.83 dB), ÷2 → 6.55 dB; `#dsp` bit-neutral over 9 renders; `AcousticEditorBrowser.js` section [6] 10/10 (drop → 77 % tracked / median 0.32 % / 0 slips in 21 s, worklet in step with the clip within 10 ms, seek, ÷2 / ×2, hear modes) | drop a phone recording of the car you selected: the trace follows the revs, Synth / Both play the car along it, Split shows the two order sheets |
