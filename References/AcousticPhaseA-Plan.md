...
Target repository: **`unassignedinbox/Slate` @ `arena/01a06c54-slate`** (tip `91072b0`, 2026-09-05). This file is
written for Slate's `References/` and follows its conventions (`CLAUDE.md`, `Session-Conventions.md`: plan first,
one commit per row, explicit-path staging, proofs in `Scratchpad/`, screenshots in `Diagnostics/`, ⚠️ deviations).

⚠️ **Logistics before anything else:** the Arena sandbox writing this plan has **no push permission on Slate**
(`git push --dry-run` → 403; `viewerPermission` empty). Two ways to execute: (a) open an Arena session on Slate
branch `arena/01a06c54-slate` and say "go A1" there (this plan copied into `References/`), or (b) grant the Arena
GitHub app access to `unassignedinbox/Slate`. Until then nothing below touches Slate.
...
1. **Access** — (a) new Arena session on Slate `arena/01a06c54-slate` with this plan pasted in, or (b) grant the Arena
   app push on `unassignedinbox/Slate`. (This session cannot commit to Slate.)
2. **Names** — `Project-Dyno`; `AudioExchange` / `WaveCodec` / `AcousticStructure` / `AcousticIntegrator` / `DynoSequence`
   / `PowertrainRecord` / `AudioMetrics`. Alternatives welcome; §7 vocabulary otherwise applies.
3. **Car TOML home** — `Projects/Project-Dyno/Content/AudioArchives/` (copied into the game at merge) **or**
...
4. **Car order** — 918 → LaFerrari → GT-R (recommended) or GT-R first as you listed.
5. **Editor feeds** — file + worklet now (as written), C++ WebSocket feed in Phase B. Or pull the WebSocket feed into A4
   (+1 row: `TelemetryInterchange`, ~200 lines: handshake + framing, no library).
6. **Reference recordings** — will you supply short clips per car (idle / steady 4 k / WOT pull / overrun), or do we judge by
   ear + order-signature checks only?
7. **miniaudio pin** — plan pins `9634bedb` (v0.11.25) per CLAUDE.md; if your local `ExternalPackages\miniaudio` is another
