...
| **A4** editor live | ~~worklet port, scopes, order diagram, TOML I/O, file feed, WAV record~~ done in A1½; ~~C++ ⇄ worklet identity, `--render` order overlay~~ done in A2 (Split / Δ on the order diagram) — remaining: **reference lane** (drop a *recording*, aligned spectrogram + order diagram over the synth's — the overlay machinery now exists; a recording needs an rpm trace instead of the pull timeline) | order diagram of the C++ `--render` matches the worklet within ±0.1 dB per order ✅ (0.0000 dB) | audible in the live preview |
