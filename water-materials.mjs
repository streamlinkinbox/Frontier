...
export const waterFragment = `
  ...
  void main(){
    ...
    vec2 p=vPosition.xz;
    // Wind/capillary-scale detail affects shading only; resolved waves come from the solver.
    vec2 micro=vec2(cos(p.x*8.1+p.y*3.7+uTime*1.7)+.42*cos(p.x*19.-p.y*11.+uTime*2.3),
      sin(p.y*9.7-p.x*4.1+uTime*1.2)+.42*sin(p.y*17.+p.x*12.-uTime*2.1));
    ...
    // Decorative focusing pattern, attenuated twice along the underwater optical path.
    float caustic=pow(abs(sin(bedPoint.x*5.+sin(bedPoint.z*3.+uTime*.35))*sin(bedPoint.z*5.+sin(bedPoint.x*3.-uTime*.27))),14.);
    transmission+=vec3(.13,.13,.085)*caustic*transmittance*transmittance;
    vec3 R=reflect(-V,N);
