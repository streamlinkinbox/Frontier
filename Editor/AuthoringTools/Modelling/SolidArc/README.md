...
Folder: `Editor/AuthoringTools/Modelling/SolidArc/`. Application name: **SolidArc**.
...
```bash
cmake -S Editor/AuthoringTools/Modelling/SolidArc -B /tmp/solidarc-build -G Ninja
cmake --build /tmp/solidarc-build
ctest --test-dir /tmp/solidarc-build --output-on-failure
```

If CMake is not available, the focused gate is dependency-free:
...
face-edit and bounded curved-root proofs. It runs 48 verifier targets and persists each target's distinct PNG where
provided. No external packages. `-Wall -Wextra -Wpedantic`.

## Direct solid modelling (Phase 34)

This increment answers the solid-to-solid workflow directly instead of treating solids as thin sheets:
...
```text
box (0,0,0) (4,4,2) --name=Block
cylinder (2,2,5) 2 2 --axis=(0,0,1) --name=Boss
# `topology` lists these two facing cap indices for this construction.
loft Block:f1 Boss:f1 --name=Transition

tweak Transition (0,0,1.5) --face=1 --name=Raised
...
triangles). `TweakVerification` contributes 41 checks and `Proofs/Phase34b_Tweak.png`. Planar single-edge and connected
edge-loop chamfers are covered by the Phase 34c/34d direct-modelling gate below.

## Planar single-edge and edge-loop chamfers (Phase 34c/34d)

`chamfer Body setback --edges=i[,j,…]` is now an all-or-nothing direct-modelling operation. On convex planar solids,
`BlendSolver::ChamferEdges` constructs the retained half-space intersection with one setback plane per selected edge;
shared endpoint intersections are therefore real mitres and non-right dihedrals remain analytic. An over-large setback is
rejected before it can cross the next boundary vertex. Curved/non-planar edges, duplicate members, and unsupported
non-convex networks refuse without consuming the source. The console commits the destination only after the complete
selection succeeds. `DirectModelingVerification` covers the console route and
`ChamferLoopVerification` contributes 18 checks plus `Proofs/Phase34c_SingleEdgeChamfer.png` and
`Proofs/Phase34d_ChamferLoop.png`.
...
| 36k | **Bounded partial cone–cylinder root chamfer.** A canonical coaxial cone/cylinder sector, including the half-turn, reconstructs one exact `V10/E15/F7` general-sector manifold with radial endpoint-cap healing; cone–cone, non-coaxial, apex, arbitrary, and branched supports remain refused. | `PartialConeCylinderChamferVerification` — 14 C++ checks; `Proofs/Phase36k_PartialConeCylinderChamfer.png` |
...
| 37a | **Bounded rolling-ball variable-radius corner fillet.** One finite straight edge between perpendicular planar supports accepts a positive linear radius law, reconstructing exact quarter-circle stations with capped `V10/E15/C30/L7/F7` topology and analytic integrated volume; partial-edge, nonlinear, G2, apex, freeform, and arbitrary-healing routes remain refused. | `VariableRadiusCornerFilletVerification` — 15 C++ checks; `Proofs/Phase37_VariableRadiusCornerFillet.png` |
| 37b | **Bounded variable support-setback corner fillet.** The same finite straight corner accepts an independent positive linear setback/clearance law, measured from each rolling tangent point to the far support boundary; the variable outer extent and rounded removal are checked analytically with capped `V10/E15/C30/L7/F7` topology. Nonlinear laws, partial edges, apexes, G2 joins, and arbitrary healing remain refused. | `VariableSetbackCornerFilletVerification` — 24 C++ checks; `Proofs/Phase37b_VariableSetbackCornerFillet.png` |
| 37c | **Bounded quadratic nonlinear-radius corner fillet.** A radius law specified at the two endpoints and middle station produces a quadratic loft through exact quarter-circle sections, with explicit first/second derivatives, curvature acceptance, analytic volume, and capped `V10/E15/C30/L7/F7` topology. G2 joins, nonlinear setbacks, partial edges, apexes, and arbitrary healing remain refused. | `NonlinearVariableRadiusCornerVerification` — 24 C++ checks; `Proofs/Phase37c_NonlinearVariableRadiusCorner.png` |
...
| 37g | **Bounded nonlinear unequal support-setback corner blend.** Independent genuinely nonlinear quadratic clearance laws on the two perpendicular supports drive three-station quadratic lofts with exact quarter-circle sections, product-of-quadratic-extents volume, capped `V10/E15/C30/L7/F7` topology, and explicit linear/equal-law refusals. Rolling-ball G2, arbitrary edge selection, freeform supports, and healing remain refused. | `NonlinearUnequalSetbackCornerVerification` — 34 C++ checks; `Proofs/Phase37g_NonlinearUnequalSetbackCorner.png` |
...
ctest now registers **76 suites** — 63 per-feature verification binaries (2,219 checks total) and 13 script smoke
