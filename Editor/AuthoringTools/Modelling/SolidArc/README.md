...
That gate compiles the C++20 kernel, console and interaction layers, then runs the Phase 34a and 34b proofs.
...
- `chamfer Body setback --edges=i` is intentionally one physical edge per command. A box/polygon edge gets a
  planar-setback bevel and the body remains closed; a comma-separated edge set is refused rather than silently
  beveling only the first edge. Curve profiles still support `chamfer Curve setback --corners=i,j`.
...
`FaceLoftVerification` runs 29 checks, `TweakVerification` runs 41 checks, and `DirectModelingVerification`
checks the closed single-edge chamfer and transactional edge-loop refusal. The focused script runs all three.
