...
The native partial-cone apex chamfer is a distinct selection layer for the exact partial-revolve source:

- `ClassifyPartialConeApexChamferVertex` accepts only the unique axis apex of native `V4/E6/C8/L3/F3`
  topology with three single-loop revolution faces and an equal-radius circular base rim;
- it unwraps and retains the actual native sweep for every strict partial sector, derives the base,
  normalized axis, radius, and height, and validates a positive axial set-back without mutating the source;
- `PartialConeApexVertexChamferVerification` performs 29 checks over a distinct 120-degree 5-by-7
  source, covering exact extraction, deterministic dispatch, separate `V6/E9/C18/L5/F5` planar-cap
  reconstruction, exact frustum radii, sector volume, outward normals, source immutability, and
  non-apex/invalid/full-turn/cylinder/malformed refusals;
- `Tools/Build/CheckSolidArc.sh` compiles and runs the verifier, and persists
  `Proofs/Phase38v_PartialConeApexVertexChamfer.png`. This is a new planar-cap proof, not a duplicate
  of the complete-cone or 60/180/270-degree spherical-filleting images.

This does not prove arbitrary vertex-selected chamfers, mixed cone/plane/cylinder supports,
freeform apexes, or general intersection/trim/sew healing. The bounded plan is
`docs/PLAN_PartialConeApexVertexChamfer.md`.
...
3. General vertex-selected apex **fillets and chamfers** remain bounded; Stages 4b/4q–4t cover
   canonical cone and partial-cone spherical caps, while Stages 4u–4v cover complete and native
   partial-cone axial planar caps. Mixed cone/plane/cylinder supports, freeform apexes, and arbitrary
   apex selections remain refused.
...
## Phase 39h — full-turn equal-radius bicone chamfer with independent set-backs (2026-09-24)

This is a separate full-turn equal-radius route, not a renamed equal-setback or unequal-radius
fixture:

- `EqualRadiusBiconeUnequalSetbackChamferVerification` builds the native point-contact source with
  exact `V5/E6/C12/L4/F4` topology, equal radii 3.6/3.6, heights 6.2/4.7, and lower/upper
  set-backs 0.7/1.1.
- `ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex` derives the shared apex, canonical coaxial
  axis, equal support radius, support heights, and independent set-backs while refusing equal
  set-backs and unsupported source families transactionally.
- `ReconstructEqualRadiusBiconeUnequalSetbackChamfer` returns `V6/E9/C18/L5/F5` with three cone
  faces and two planar base faces. The verifier checks the exact three-frustum volume identity,
  contact radii, outward normals, source immutability, deterministic dispatch, and complete/partial/
  half/reflex refusal boundaries.
- The durable proof is `Proofs/Phase39h_EqualRadiusBiconeUnequalSetbackChamfer.png`; 36 checks pass
  in the focused direct gate, and the verifier/target are registered in `CheckSolidArc.sh` and CMake.

Equal set-backs remain on Phase 39e. This does not prove partial/half/reflex sectors as accepted
full-turn inputs, unequal radii, mixed cone/plane/cylinder, oblique/non-coaxial or freeform supports,
...
## Phase 39k — half-turn equal-radius bicone apex toroidal fillet (2026-09-24)

This is a separate exact-half-turn toroidal route, not a partial-sector or complete-turn alias:

- `HalfTurnEqualRadiusBiconeApexFilletVerification` builds the capped half-turn source with exact
  `V7/E11/C22/L6/F6` topology, equal radii 3.6/3.6, heights 6.2/4.7, and fillet radius 0.58.
- `ClassifyHalfTurnEqualRadiusBiconeApexFilletVertex` derives the shared apex, canonical coaxial axis,
  equal support radius, support heights, and exactly `pi` sweep while refusing unsupported source
  families transactionally.
- `ReconstructHalfTurnEqualRadiusBiconeApexFillet` solves the tangent contacts and analytic torus,
  returns `V10/E15/C30/L7/F7` with two cones, one torus, two axial base planes, and two radial sector
  planes, and checks radial-cap closure, outward normals, torus metadata, and the cone-plus-torus
  identity scaled by `sweep / (2 pi)`.
- The durable proof is `Proofs/Phase39k_HalfTurnEqualRadiusBiconeApexFillet.png`; 33 checks
...
## Phase 43 — elliptical-prism face offset (2026-09-25)

This phase targets a distinct curved-profile modelling domain rather than another cone/bicone blend
variant or a circular-radius change:

- `EllipticalPrismFaceOffsetVerification` constructs a rational axis-aligned ellipse with major
  radius 6, minor radius 3, height 5, and positive upper-cap offset distance 2. The source and
  result retain exact `V2/E3/C6/L3/F3` genus-zero topology.
- `FaceEditSolver::OffsetExtrudedEllipticalPrism` validates the selected upper planar cap, two Z
  levels, two closed rational degree-2 ellipse rims, the major/minor bounds, axis orientation, and
  exact profile samples before rebuilding the ellipse at the extended height. No polygonal
  approximation or healing fallback is used.
- The verifier checks the pi-area volume identity within the NURBS circular/ellipse tolerance,
  both ellipse rims, analytic planar/extrusion support classifications, public `OffsetFace`
  dispatch, source immutability, and circular/polygonal/holed/freeform/side/invalid/malformed
  refusal boundaries.
- The durable proof is `Proofs/Phase43_EllipticalPrismFaceOffset.png`; 20 checks pass in the
  focused direct gate, and the verifier/target/proof are registered in `CheckSolidArc.sh` and
  CMake.

This does not prove circular cylinders through this API, rotated ellipses, tilted/non-prismatic
supports, arbitrary conics, freeform profiles, multi-loop profiles, lower/side-face offsets,
healing, or general shell/thicken behavior. The bounded plan is
