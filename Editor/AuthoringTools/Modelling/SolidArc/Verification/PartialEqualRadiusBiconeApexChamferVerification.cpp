//=============================================================================================================================================
// SolidArc · Phase 39d · bounded partial equal-radius bicone apex chamfer
//=============================================================================================================================================
...
[[nodiscard]] bool OutputNormals(const BrepBody& Body, const PartialEqualRadiusBiconeApexChamferSpecification& S) noexcept
{
...
int main()
{
    VerificationPanel Panel("SolidArc · bounded partial equal-radius bicone apex chamfer");
    const Fixture F{ { 0, 0, 0 }, { 0, 0, 1 }, 3.8, 3.8, 6.1, 4.6, 105.0 * ScalarCriteria::Pi / 180.0 };
    const double SetBack = 0.85;
    const auto SourceDeliver = MakeSource(F);
    Panel.Expect("The distinct capped equal-radius partial source is constructed", static_cast<bool>(SourceDeliver));
    ...
    Panel.Expect("The source is a closed genus-zero equal-radius partial bicone", Before.Solid() && Before.Hulls == 1 &&
    ...
    const auto Candidate = BlendSolver::ClassifyPartialEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, SetBack);
    Panel.Expect("The partial equal-apex dispatch accepts the selected vertex", static_cast<bool>(Candidate));
    if (Candidate)
    {
        ...
        Panel.Expect("The partial dispatch is deterministic and transactional",
                     BlendSolver::ClassifyPartialEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, SetBack) &&
                     SameSource(Source, Snapshot, Before));
    }
    Panel.Section("Partial equal three-frustum and radial-cap reconstruction");
    const auto Result = Candidate ? BlendSolver::ReconstructPartialEqualRadiusBiconeApexChamfer(Candidate.Payload)
                                  : Deliver<BrepBody>::Reject(RefusalReason::Unsupported, "no eligible partial apex");
    Panel.Expect("The partial equal chamfer reconstructs one solid", Result && Result.Payload.Validate().Solid());
    if (Result)
    {
        ...
        Panel.Within("The partial volume is the exact sector-scaled three-frustum identity",
    ...
    Panel.Section("Strict partial equal-apex refusal boundaries");
    Panel.Expect("A non-apex vertex refuses", !BlendSolver::ClassifyPartialEqualRadiusBiconeApexChamferVertex(Source, 0, SetBack));
    Panel.Expect("An out-of-range vertex refuses", !BlendSolver::ClassifyPartialEqualRadiusBiconeApexChamferVertex(Source, 99, SetBack));
    Panel.Expect("Zero, negative, and non-finite set-backs refuse",
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, 0.0) &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, -0.2) &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, std::numeric_limits<double>::infinity()));
    Panel.Expect("A consuming set-back refuses", !BlendSolver::ClassifyPartialEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, F.UpperHeight));
    auto FullCandidate = Candidate.Payload; FullCandidate.SweepAngle = ScalarCriteria::TwoPi;
    Panel.Expect("A complete-turn specification refuses", !BlendSolver::ReconstructPartialEqualRadiusBiconeApexChamfer(FullCandidate));
    ...
    Panel.Expect("An unequal-radius partial bicone remains outside this equal route", UnequalSource &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeApexChamferVertex(UnequalSource.Payload, ApexVertex, SetBack));
    const auto Cylinder = BrepBody::Cylinder(F.Apex - F.Axis * F.LowerHeight, F.Axis, F.LowerRadius, F.LowerHeight);
    Panel.Expect("A cylinder remains outside partial equal-apex dispatch", Cylinder &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeApexChamferVertex(Cylinder.Payload, 0, SetBack));
    BrepBody Malformed = Source; Malformed.Edges[0].Coedges.push_back(Malformed.Edges[0].Coedges.front());
    Panel.Expect("A malformed partial source refuses transactionally", !BlendSolver::ClassifyPartialEqualRadiusBiconeApexChamferVertex(Malformed, ApexVertex, SetBack));
    ...
    const bool Added = SourceDeliver && Result &&
        Host.Document().AddBody("PartialEqualApexSource", Source.Transformed(Mat4::Translation({ -9, 0, 0 }))).Identity > 0 &&
        Host.Document().AddBody("PartialEqualApexChamfer", Result.Payload.Transformed(Mat4::Translation({ 9, 0, 0 }))).Identity > 0;
    const bool Rendered = Added && Host.Execute("show shading flat") && Host.Execute("view iso") &&
        ...
        Host.Execute("render Phase39d_PartialEqualBiconeApexChamfer");
    Panel.Expect("The partial equal-apex proof render completes", Rendered);
    Panel.Expect("The partial equal-apex proof PNG is visible", std::filesystem::exists(Proof) && std::filesystem::file_size(Proof, Error) > 100000);
