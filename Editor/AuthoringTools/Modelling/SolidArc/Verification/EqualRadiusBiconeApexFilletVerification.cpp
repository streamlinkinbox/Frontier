//=============================================================================================================================================
// SolidArc · Stage 4y · bounded equal-radius coaxial bicone apex toroidal fillet
//=============================================================================================================================================
...
[[nodiscard]] Tangency Derive(const EqualRadiusBiconeApexFilletSpecification& S) noexcept
{
    const Vec3 Axis = S.Axis.Normalised();
    const Vec3 Radial = Workplane::FromNormal(S.Apex, Axis).AxisX.Normalised();
    Tangency T;
    T.LowerAngle = std::atan2(S.Radius, S.LowerHeight);
    T.UpperAngle = std::atan2(S.Radius, S.UpperHeight);
...
[[nodiscard]] bool OutwardNormals(const BrepBody& Body, const EqualRadiusBiconeApexFilletSpecification& S) noexcept
{
    const Vec3 Axis = S.Axis.Normalised();
...
[[nodiscard]] bool ExactMetadata(const BrepBody& Body, const EqualRadiusBiconeApexFilletSpecification& S) noexcept
{
    const Tangency T = Derive(S);
    const Vec3 Axis = S.Axis.Normalised();
    const Vec3 LowerBase = S.Apex - Axis * S.LowerHeight;
...
[[nodiscard]] bool TangentContacts(const BrepBody& Body, const EqualRadiusBiconeApexFilletSpecification& S) noexcept
{
    const Tangency T = Derive(S);
...
int main()
{
    VerificationPanel Panel("SolidArc · bounded equal-radius coaxial bicone apex toroidal fillet");
    const Fixture F{ { 0, 0, 0 }, { 0, 0, 1 }, 3.5, 3.5, 6.0, 4.5 };
    const double FilletRadius = 0.65;
    const Deliver<BrepBody> SourceDeliver = MakeSource(F);
    Panel.Expect("The distinct equal-radius source is constructed", static_cast<bool>(SourceDeliver));
    ...
    const auto Candidate = BlendSolver::ClassifyEqualRadiusBiconeApexFilletVertex(Source, ApexVertex, FilletRadius);
    Panel.Expect("The equal-radius apex fillet dispatch accepts the selected vertex", static_cast<bool>(Candidate));
    if (Candidate)
    {
        ...
        Panel.Within("The lower radius is extracted exactly", std::fabs(S.Radius - F.LowerRadius), 1e-12);
        Panel.Within("The upper radius is extracted exactly", std::fabs(S.Radius - F.UpperRadius), 1e-12);
        ...
        Panel.Expect("The support radii are exactly equal", std::fabs(S.Radius - F.LowerRadius) <= 1e-9);
        Panel.Expect("The toroidal dispatch is deterministic", BlendSolver::ClassifyEqualRadiusBiconeApexFilletVertex(
            Source, ApexVertex, FilletRadius) && SameSource(Source, Snapshot, Before));
    }

    Panel.Section("Analytic equal-radius bicone toroidal apex fillet");
    const auto Result = Candidate ? BlendSolver::ReconstructEqualRadiusBiconeApexFillet(Candidate.Payload)
    ...
    if (Result)
    {
        ...
        Panel.Expect("The two equal-radius cones, torus, and caps have outward normals", OutwardNormals(Result.Payload, Candidate.Payload));
        Panel.Expect("The torus and equal support metadata are exact", ExactMetadata(Result.Payload, Candidate.Payload));
        Panel.Expect("The torus contacts both equal cone generators tangentially", TangentContacts(Result.Payload, Candidate.Payload));
        ...
        const double Expected = ScalarCriteria::Pi * (Candidate.Payload.LowerHeight - LowerDepth) *
            (Candidate.Payload.Radius * Candidate.Payload.Radius + Candidate.Payload.Radius * LowerRim +
             LowerRim * LowerRim) / 3.0 + ScalarCriteria::Pi * MeridionalIntegral +
            ScalarCriteria::Pi * (Candidate.Payload.UpperHeight - UpperDepth) *
            (UpperRim * UpperRim + UpperRim * Candidate.Payload.Radius +
    ...
    Panel.Section("Equal-radius toroidal apex refusal boundaries");
    Panel.Expect("A non-apex vertex refuses", !BlendSolver::ClassifyEqualRadiusBiconeApexFilletVertex(Source, 0, FilletRadius));
    Panel.Expect("An out-of-range vertex refuses", !BlendSolver::ClassifyEqualRadiusBiconeApexFilletVertex(Source, 99, FilletRadius));
    Panel.Expect("Zero, negative, and non-finite radii refuse",
                 !BlendSolver::ClassifyEqualRadiusBiconeApexFilletVertex(Source, ApexVertex, 0.0) &&
                 !BlendSolver::ClassifyEqualRadiusBiconeApexFilletVertex(Source, ApexVertex, -0.2) &&
                 !BlendSolver::ClassifyEqualRadiusBiconeApexFilletVertex(Source, ApexVertex, std::numeric_limits<double>::infinity()));
    Panel.Expect("A consuming toroidal radius refuses",
                 !BlendSolver::ClassifyEqualRadiusBiconeApexFilletVertex(Source, ApexVertex, 100.0));
    const Fixture UnequalRadius{ F.Apex, F.Axis, F.LowerRadius, 2.5, F.LowerHeight, F.UpperHeight };
    const auto UnequalSource = MakeSource(UnequalRadius);
    Panel.Expect("An unequal-radius bicone remains outside this equal route", UnequalSource &&
                 !BlendSolver::ClassifyEqualRadiusBiconeApexFilletVertex(UnequalSource.Payload, ApexVertex, FilletRadius));
    const auto Cylinder = BrepBody::Cylinder(F.Apex - F.Axis * F.LowerHeight, F.Axis, F.LowerRadius, F.LowerHeight);
    Panel.Expect("A cylinder remains outside equal-radius apex filleting", Cylinder &&
                 !BlendSolver::ClassifyEqualRadiusBiconeApexFilletVertex(Cylinder.Payload, 0, FilletRadius));
    BrepBody Malformed = Source;
    Malformed.Edges[0].Coedges.push_back(Malformed.Edges[0].Coedges.front());
    Panel.Expect("A malformed source refuses transactionally", !BlendSolver::ClassifyEqualRadiusBiconeApexFilletVertex(
    ...
    const bool Added = SourceDeliver && Result &&
        Host.Document().AddBody("EqualApexFilletSource", Source.Transformed(Mat4::Translation({ -9, 0, 0 }))).Identity > 0 &&
        Host.Document().AddBody("EqualToroidalFillet", Result.Payload.Transformed(Mat4::Translation({ 9, 0, 0 }))).Identity > 0;
    const bool Rendered = Added && Host.Execute("show shading flat") && Host.Execute("view iso") &&
        ...
        Host.Execute("render Phase38z_EqualRadiusBiconeApexFillet");
    Panel.Expect("The equal-radius toroidal apex proof render completes", Rendered);
    Panel.Expect("The equal-radius toroidal apex proof PNG is visible",
