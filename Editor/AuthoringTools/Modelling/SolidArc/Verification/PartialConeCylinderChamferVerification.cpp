//=============================================================================================================================================
// SolidArc · Phase 36k · bounded partial cone–cylinder root chamfer
//
...
[[nodiscard]] double AddedWedge(double BaseRadius, double RootRadius, double ConeHeight, double SetBack,
                                double SweepAngle) noexcept
{
    const double Slant = std::hypot(ConeHeight, BaseRadius - RootRadius);
    const double Axial = SetBack * ConeHeight / Slant;
    const double Contact = RootRadius + (BaseRadius - RootRadius) * Axial / ConeHeight;
    const double Full = ScalarCriteria::Pi * (Contact - RootRadius) * SetBack *
        (Contact + 2.0 * RootRadius) / 3.0;
...
int main()
{
    VerificationPanel Panel("SolidArc · Phase 36k · partial cone–cylinder root chamfer");
    constexpr double BaseRadius = 10.0, RootRadius = 5.0, ConeHeight = 8.0, CylinderHeight = 7.0;
    ...
    Panel.Expect("The half-turn cone–cylinder chamfer commits exact V10/E14/F6 topology",
    ...
    Panel.Expect("The partial cone–cylinder chamfer commits one complete chain", Result && Result.Payload.Validate().Solid() && Applied == 1);
    if (Result)
    {
        ...
        const double Expected = AddedWedge(BaseRadius, RootRadius, ConeHeight, SetBack, SweepAngle);
        Panel.Within("The sector wedge follows the exact cone–cylinder meridian integral",
    ...
    if (!Members.empty())
        Panel.Expect("Zero and consuming setbacks refuse without mutating the source",
                     ...
                     !BlendSolver::ChamferEdge(Source, Members.front(), CylinderHeight) &&
    ...
    const bool ConsoleCommit = Added && !Members.empty() &&
        Host.Execute("chamfer PartialConeCylinderSource 0.5 --edges=" + std::to_string(Members.front()) + " --name=PartialConeCylinderChamfer") &&
        Host.Document().Find("PartialConeCylinderChamfer") &&
        Host.Document().Find("PartialConeCylinderChamfer")->Body.Validate().Solid();
    Panel.Expect("The console commits the bounded cone–cylinder route", ConsoleCommit);

    const std::filesystem::path Proof = std::filesystem::path(SOLIDARC_PROOF_FOLDER) / "Phase36k_PartialConeCylinderChamfer.png";
    ...
    const bool Rendered = SourceResult && ProofResult &&
        ...
        ProofHost.Document().AddBody("ChamferedPartialConeCylinder", ProofResult.Payload.Transformed(Mat4::Translation({ 11, 0, 0 }))).Identity > 0 &&
        ProofHost.Execute("view iso") && ProofHost.Execute("view fit") && ProofHost.Execute("render Phase36k_PartialConeCylinderChamfer");
