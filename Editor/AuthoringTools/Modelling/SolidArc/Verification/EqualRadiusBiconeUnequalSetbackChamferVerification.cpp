//=============================================================================================================================================
// SolidArc · Phase 39h · bounded full-turn equal-radius bicone chamfer with independent set-backs
//=============================================================================================================================================
...
[[nodiscard]] Deliver<BrepBody> MakeProfileRevolvedSector(const Fixture& F) noexcept
{
    const Vec3 Axis = F.Axis.Normalised();
    const Vec3 Radial = Workplane::FromNormal(F.Apex, Axis).AxisX.Normalised();
    const Vec3 LowerBase = F.Apex - Axis * F.LowerHeight;
    const Vec3 UpperBase = F.Apex + Axis * F.UpperHeight;
    const Deliver<NurbsCurve> Profile = NurbsCurve::Polyline({
        LowerBase, LowerBase + Radial * F.LowerRadius, F.Apex,
        UpperBase + Radial * F.UpperRadius, UpperBase }, true);
    return Profile ? BrepBody::Revolve(Profile.Payload, F.Apex, Axis, F.Sweep)
                   : Deliver<BrepBody>::Reject(RefusalReason::DegenerateInput, "sector profile is degenerate");
}
...
[[nodiscard]] bool OutwardNormals(const BrepBody& Body,
                                  const EqualRadiusBiconeUnequalSetbackChamferSpecification& S) noexcept
{
    ...
    for (size_t I = 0; I < Body.Faces.size(); ++I)
    {
        ...
        if (Surface.Classification == SurfaceClassification::Cone)
        {
            ...
            const double RadiusDelta = Lower ? S.Radius - LowerContactRadius
                : (Bridge ? LowerContactRadius - UpperContactRadius : UpperContactRadius - S.Radius);
...
int main()
{
    VerificationPanel Panel("SolidArc · bounded full-turn equal-radius bicone chamfer with independent set-backs");
    const Fixture F{ { 0, 0, 0 }, { 0, 0, 1 }, 3.6, 3.6, 6.2, 4.7 };
    const double LowerSetBack = 0.7;
    const double UpperSetBack = 1.1;
    const Deliver<BrepBody> SourceDeliver = MakeNativeBicone(F);
    Panel.Expect("The distinct equal-radius independent-setback source is constructed", static_cast<bool>(SourceDeliver));
    ...
    Panel.Expect("The source is the two-hull equal-radius point-contact bicone", Before.Solid() && Before.Hulls == 2 &&
    ...
    const Deliver<EqualRadiusBiconeUnequalSetbackChamferSpecification> Candidate =
        BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex,
                                                                          LowerSetBack, UpperSetBack);
    Panel.Expect("The equal-radius independent-setback apex dispatch accepts the selected vertex", static_cast<bool>(Candidate));
    if (Candidate)
    {
        ...
        Panel.Expect("The equal-radius independent-setback classification is transactional", SameSource(Source, Snapshot, Before));
        const auto Repeat = BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(
            Source, ApexVertex, LowerSetBack, UpperSetBack);
        Panel.Expect("The equal-radius independent-setback dispatch is deterministic", Repeat &&
    ...
    const auto Result = Candidate ? BlendSolver::ReconstructEqualRadiusBiconeUnequalSetbackChamfer(Candidate.Payload)
                                  : Deliver<BrepBody>::Reject(RefusalReason::Unsupported, "no eligible apex");
    Panel.Expect("The equal-radius independent-setback reconstruction is a solid", Result && Result.Payload.Validate().Solid());
    if (Result)
    {
    ...
    Panel.Section("Equal-radius independent-setback refusal boundaries");
    Panel.Expect("A non-apex vertex refuses", !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(
        Source, 0, LowerSetBack, UpperSetBack));
    Panel.Expect("An out-of-range vertex refuses", !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(
        Source, 99, LowerSetBack, UpperSetBack));
    Panel.Expect("Equal set-backs remain on the Phase 39e route", !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(
        Source, ApexVertex, 1.0, 1.0));
    Panel.Expect("Zero, negative, and non-finite set-backs refuse",
                 !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, 0.0, UpperSetBack) &&
                 !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, LowerSetBack, -0.2) &&
                 !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, LowerSetBack,
                                                                                   std::numeric_limits<double>::infinity()));
    Panel.Expect("A consuming lower or upper set-back refuses",
                 !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, F.LowerHeight, UpperSetBack) &&
                 !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, LowerSetBack, F.UpperHeight));
    ...
    Panel.Expect("An unequal-radius source remains outside the equal route", UnequalSource &&
                 !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(UnequalSource.Payload, ApexVertex,
                                                                                    LowerSetBack, UpperSetBack));
    ...
    Panel.Expect("A strict partial sector remains outside the full-turn route", PartialSource &&
                 !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(PartialSource.Payload, 2,
                                                                                    LowerSetBack, UpperSetBack));
    PartialFixture.Sweep = ScalarCriteria::Pi;
    const auto HalfSource = MakeProfileRevolvedSector(PartialFixture);
    Panel.Expect("The half-turn refusal fixture is constructed", static_cast<bool>(HalfSource));
    Panel.Expect("A half-turn sector remains outside the full-turn route", HalfSource &&
                 !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(HalfSource.Payload, 2,
                                                                                    LowerSetBack, UpperSetBack));
    PartialFixture.Sweep = 1.25 * ScalarCriteria::Pi;
    const auto ReflexSource = MakePartialBicone(PartialFixture);
    Panel.Expect("A reflex sector remains outside the full-turn route", ReflexSource &&
                 !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(ReflexSource.Payload, 2,
                                                                                    LowerSetBack, UpperSetBack));
    ...
    Panel.Expect("A cylinder remains outside independent-setback dispatch", Cylinder &&
                 !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(Cylinder.Payload, 0,
                                                                                    LowerSetBack, UpperSetBack));
    BrepBody Malformed = Source;
    Malformed.Edges[0].Coedges.push_back(Malformed.Edges[0].Coedges.front());
    Panel.Expect("A malformed source refuses transactionally", !BlendSolver::ClassifyEqualRadiusBiconeUnequalSetbackChamferVertex(
    ...
    const bool Rendered = Added && Host.Execute("show shading flat") && Host.Execute("view iso") &&
        ...
        Host.Execute("render Phase39h_EqualRadiusBiconeUnequalSetbackChamfer");
    Panel.Expect("The equal-radius independent-setback proof render completes", Rendered);
    Panel.Expect("The equal-radius independent-setback proof PNG is visible",
