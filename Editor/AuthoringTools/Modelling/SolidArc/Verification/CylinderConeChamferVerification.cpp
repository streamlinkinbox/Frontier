//=============================================================================================================================================
// SolidArc · Phase 36g · bounded complete cylinder–cone boss-root chamfer
//
...
[[nodiscard]] double ChamferDelta(double BossRadius, double TopRadius, double ConeHeight, double SetBack) noexcept
{
    const double Slant = std::hypot(ConeHeight, TopRadius - BossRadius);
    const double Axial = SetBack * ConeHeight / Slant;
    const double Contact = BossRadius + (TopRadius - BossRadius) * Axial / ConeHeight;
    // Integrate the squared-radius difference between the new straight band and the old cylinder/cone boundary.
    const double EndSum = BossRadius * BossRadius + BossRadius * Contact + Contact * Contact;
    return ScalarCriteria::Pi * SetBack * (EndSum / 3.0 - BossRadius * BossRadius);
}

struct Fixture
{
    double OuterRadius = 0.0, ShoulderHeight = 0.0, BossRadius = 0.0, CylinderHeight = 0.0;
    double TopRadius = 0.0, ConeHeight = 0.0, SetBack = 0.0;
    ...
    Fixture(double Outer, double Shoulder, double Boss, double Cylinder, double Top, double Cone, double Back) noexcept
        : OuterRadius(Outer), ShoulderHeight(Shoulder), BossRadius(Boss), CylinderHeight(Cylinder),
          TopRadius(Top), ConeHeight(Cone), SetBack(Back)
    {
...
[[nodiscard]] bool ExactSupportSet(const BrepBody& Body, const Fixture& F) noexcept
{
    ...
    bool ChamferBand = false, RetainedCone = false, RetainedCylinder = false;
    const double Slant = std::hypot(F.ConeHeight, F.TopRadius - F.BossRadius);
    const double Axial = F.SetBack * F.ConeHeight / Slant;
    const double Contact = F.BossRadius + (F.TopRadius - F.BossRadius) * Axial / F.ConeHeight;
    for (const BrepFace& Face : Body.Faces)
    {
        ...
        {
            ++Cylinders;
            RetainedCylinder = RetainedCylinder ||
                std::fabs(Surface.RadiusMajor - F.BossRadius) < 1e-9;
        }
        ...
        {
            ++Cones;
            ChamferBand = ChamferBand ||
                ((std::fabs(Surface.RadiusMajor - F.BossRadius) < 1e-9 && std::fabs(Surface.RadiusMinor - Contact) < 1e-9) ||
                 (std::fabs(Surface.RadiusMinor - F.BossRadius) < 1e-9 && std::fabs(Surface.RadiusMajor - Contact) < 1e-9));
            RetainedCone = RetainedCone ||
                ((std::fabs(Surface.RadiusMajor - Contact) < 1e-9 && std::fabs(Surface.RadiusMinor - F.TopRadius) < 1e-9) ||
                 (std::fabs(Surface.RadiusMinor - Contact) < 1e-9 && std::fabs(Surface.RadiusMajor - F.TopRadius) < 1e-9));
        }
        else if (Surface.Classification == SurfaceClassification::Plane) ++Planes;
    }
    return Cylinders == 2 && Revolutions == 1 && Cones == 2 && Planes == 2 && RetainedCylinder &&
           ChamferBand && RetainedCone;
}
}

int main()
{
    VerificationPanel Panel("SolidArc · Phase 36g · complete cylinder–cone boss-root chamfer");
    ...
    Panel.Section("Exact retained supports and conical setback band");
    for (const Fixture* Case : { &Narrowing, &Flaring })
    {
        ...
        Panel.Expect("The complete cylinder-cone chamfer commits exactly one edge", Result && After.Solid() && Applied == 1);
        Panel.Expect("The result is one genus-zero manifold V6/E11/F7 solid", Result && After.Hulls == 1 && After.Genus == 0 &&
                     After.Vertices == 6 && After.Edges == 11 && After.Faces == 7 && After.OpenEdges == 0 &&
                     After.NonManifoldEdges == 0 && After.MisorientedEdges == 0);
        if (Result)
        {
            const double ExpectedDelta = ChamferDelta(Case->BossRadius, Case->TopRadius, Case->ConeHeight, Case->SetBack);
            Panel.Within("The mixed curved-support volume follows the exact meridian square-radius integral",
                         std::fabs((After.Volume - Before.Volume) - ExpectedDelta) / std::max(1e-9, std::fabs(ExpectedDelta)), 2e-3);
            Panel.Expect("The retained cylinders, shoulder, two cone bands and caps remain analytic", ExactSupportSet(Result.Payload, *Case));
    ...
    if (Narrowing.Source && Narrowing.Edge >= 0)
    {
        ...
        Panel.Expect("Zero, negative, and cylinder-consuming setbacks refuse transactionally",
                     ...
                     !BlendSolver::ChamferEdge(Narrowing.Source.Payload, Narrowing.Edge, Narrowing.CylinderHeight) &&
                     SameSource(Narrowing.Source.Payload, Before));
        Panel.Expect("A setback consuming the conical support refuses transactionally",
                     !BlendSolver::ChamferEdge(Narrowing.Source.Payload, Narrowing.Edge,
                                                std::hypot(Narrowing.ConeHeight, Narrowing.TopRadius - Narrowing.BossRadius)) &&
                     SameSource(Narrowing.Source.Payload, Before));
    }
    ...
    const bool ConsoleCommit = Added && Host.Execute("chamfer CylinderConeSource 0.5 --edges=" + std::to_string(Narrowing.Edge) +
                                                     " --name=CylinderConeChamfer") &&
                               Host.Document().Find("CylinderConeChamfer") &&
                               Host.Document().Find("CylinderConeChamfer")->Body.Validate().Solid();
    Panel.Expect("The console commits the bounded cylinder-cone route", ConsoleCommit);

    const std::filesystem::path Proof = std::filesystem::path(SOLIDARC_PROOF_FOLDER) / "Phase36g_CylinderConeChamfer.png";
    ...
    const bool Rendered = Narrowing.Source && ProofResult &&
        ...
        ProofHost.Document().AddBody("ChamferedCylinderCone", ProofResult.Payload.Transformed(Mat4::Translation({ 11, 0, 0 }))).Identity > 0 &&
        ProofHost.Execute("view iso") && ProofHost.Execute("view fit") && ProofHost.Execute("render Phase36g_CylinderConeChamfer");
    Panel.Expect("The cylinder-cone source/result proof render completes", Rendered);
    Panel.Expect("The cylinder-cone chamfer proof PNG is visible", std::filesystem::exists(Proof) && std::filesystem::file_size(Proof, Error) > 100000);
