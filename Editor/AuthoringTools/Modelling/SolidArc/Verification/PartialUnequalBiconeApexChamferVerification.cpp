//=============================================================================================================================================
// SolidArc · Phase 39a · bounded partial unequal-radius bicone apex chamfer
//=============================================================================================================================================
...
[[nodiscard]] bool OutputNormals(const BrepBody& Body, const PartialUnequalConeApexChamferSpecification& S) noexcept
{
...
int main()
{
    VerificationPanel Panel("SolidArc · bounded partial unequal-radius bicone apex chamfer");
    const Fixture F{ { 0, 0, 0 }, { 0, 0, 1 }, 4.2, 2.8, 6.4, 4.8, 5.0 * ScalarCriteria::Pi / 9.0 };
    ...
    const auto Candidate = BlendSolver::ClassifyPartialUnequalConeApexChamferVertex(Source, ApexVertex, SetBack);
    Panel.Expect("The partial unequal-apex dispatch accepts the selected vertex", static_cast<bool>(Candidate));
    if (Candidate)
    {
        ...
        Panel.Expect("The partial dispatch is deterministic and transactional",
                     BlendSolver::ClassifyPartialUnequalConeApexChamferVertex(Source, ApexVertex, SetBack) &&
                     SameSource(Source, Snapshot, Before));
    }
    Panel.Section("Partial three-frustum and radial-cap reconstruction");
    const auto Result = Candidate ? BlendSolver::ReconstructPartialUnequalConeApexChamfer(Candidate.Payload)
                                  : Deliver<BrepBody>::Reject(RefusalReason::Unsupported, "no eligible partial apex");
    Panel.Expect("The partial unequal chamfer reconstructs one solid", Result && Result.Payload.Validate().Solid());
    if (Result)
    {
    ...
    Panel.Section("Strict partial unequal-apex refusal boundaries");
    Panel.Expect("A non-apex vertex refuses", !BlendSolver::ClassifyPartialUnequalConeApexChamferVertex(Source, 0, SetBack));
    Panel.Expect("An out-of-range vertex refuses", !BlendSolver::ClassifyPartialUnequalConeApexChamferVertex(Source, 99, SetBack));
    Panel.Expect("Zero, negative, and non-finite set-backs refuse",
                 !BlendSolver::ClassifyPartialUnequalConeApexChamferVertex(Source, ApexVertex, 0.0) &&
                 !BlendSolver::ClassifyPartialUnequalConeApexChamferVertex(Source, ApexVertex, -0.2) &&
                 !BlendSolver::ClassifyPartialUnequalConeApexChamferVertex(Source, ApexVertex, std::numeric_limits<double>::infinity()));
    Panel.Expect("A consuming set-back refuses", !BlendSolver::ClassifyPartialUnequalConeApexChamferVertex(Source, ApexVertex, F.UpperHeight));
    auto FullCandidate = Candidate.Payload; FullCandidate.SweepAngle = ScalarCriteria::TwoPi;
    Panel.Expect("A complete-turn specification refuses", !BlendSolver::ReconstructPartialUnequalConeApexChamfer(FullCandidate));
    ...
    Panel.Expect("An equal-radius partial bicone remains outside this unequal route", EqualSource &&
                 !BlendSolver::ClassifyPartialUnequalConeApexChamferVertex(EqualSource.Payload, ApexVertex, SetBack));
    const auto Cylinder = BrepBody::Cylinder(F.Apex - F.Axis * F.LowerHeight, F.Axis, F.LowerRadius, F.LowerHeight);
    Panel.Expect("A cylinder remains outside partial unequal-apex dispatch", Cylinder &&
                 !BlendSolver::ClassifyPartialUnequalConeApexChamferVertex(Cylinder.Payload, 0, SetBack));
    BrepBody Malformed = Source; Malformed.Edges[0].Coedges.push_back(Malformed.Edges[0].Coedges.front());
    Panel.Expect("A malformed partial source refuses transactionally", !BlendSolver::ClassifyPartialUnequalConeApexChamferVertex(Malformed, ApexVertex, SetBack));
