//=============================================================================================================================================
// SolidArc · Phase 39m · bounded half-turn equal-radius bicone apex chamfer
//=============================================================================================================================================
...
[[nodiscard]] bool OutputNormals(const BrepBody& Body, const HalfTurnEqualRadiusBiconeApexChamferSpecification& S) noexcept
{
    ...
    const double LowerQ = S.Radius * S.SetBack / S.LowerHeight;
    const double UpperQ = S.Radius * S.SetBack / S.UpperHeight;
    ...
    for (size_t I = 0; I < Body.Faces.size(); ++I)
    {
        ...
        if (Surface.Classification == SurfaceClassification::Cone)
        {
            ...
            const double Delta = Lower ? S.Radius - LowerQ : (Bridge ? LowerQ - UpperQ : UpperQ - S.Radius);
...
int main()
{
    VerificationPanel Panel("SolidArc · bounded half-turn equal-radius bicone apex chamfer");
    const Fixture F{ { 0, 0, 0 }, { 0, 0, 1 }, 3.8, 3.8, 6.1, 4.6, ScalarCriteria::Pi };
    const double SetBack = 0.85;
    ...
    Panel.Expect("The source is a closed genus-zero equal-radius half-turn bicone", Before.Solid() && Before.Hulls == 1 &&
    ...
    const auto Candidate = BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, SetBack);
    Panel.Expect("The half-turn equal-apex dispatch accepts the selected vertex", static_cast<bool>(Candidate));
    if (Candidate)
    {
        const auto& S = Candidate.Payload;
        Panel.Within("The equal radius is extracted exactly", std::fabs(S.Radius - F.LowerRadius), 1e-12);
        Panel.Within("The source upper radius matches the equal radius", std::fabs(F.UpperRadius - F.LowerRadius), 1e-12);
        ...
        Panel.Expect("The source radii are exactly equal", std::fabs(F.LowerRadius - F.UpperRadius) <= 1e-12);
        Panel.Expect("The half-turn dispatch is deterministic and transactional",
                     BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, SetBack) &&
                     SameSource(Source, Snapshot, Before));
    }
    Panel.Section("Partial equal three-frustum and radial-cap reconstruction");
    const auto Result = Candidate ? BlendSolver::ReconstructHalfTurnEqualRadiusBiconeApexChamfer(Candidate.Payload)
                                  : Deliver<BrepBody>::Reject(RefusalReason::Unsupported, "no eligible half-turn apex");
    Panel.Expect("The half-turn equal chamfer reconstructs one solid", Result && Result.Payload.Validate().Solid());
    if (Result)
    {
        ...
        const double LowerQ = Candidate.Payload.Radius * SetBack / Candidate.Payload.LowerHeight;
        const double UpperQ = Candidate.Payload.Radius * SetBack / Candidate.Payload.UpperHeight;
        ...
        Panel.Within("The half-turn volume is the exact sector-scaled three-frustum identity",
                     std::fabs(Report.Volume - FullVolume * F.Sweep / ScalarCriteria::TwoPi), 1e-1);
        Panel.Expect("The separate half-turn reconstruction preserves the source", SameSource(Source, Snapshot, Before));
    }
    Panel.Section("Strict half-turn equal-apex refusal boundaries");
    Panel.Expect("A non-apex vertex refuses", !BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(Source, 0, SetBack));
    Panel.Expect("An out-of-range vertex refuses", !BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(Source, 99, SetBack));
    Panel.Expect("Zero, negative, and non-finite set-backs refuse",
                 !BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, 0.0) &&
                 !BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, -0.2) &&
                 !BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, std::numeric_limits<double>::infinity()));
    Panel.Expect("A consuming set-back refuses", !BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(Source, ApexVertex, F.UpperHeight));
    auto FullCandidate = Candidate.Payload; FullCandidate.SweepAngle = ScalarCriteria::TwoPi;
    Panel.Expect("A complete-turn specification refuses", !BlendSolver::ReconstructHalfTurnEqualRadiusBiconeApexChamfer(FullCandidate));
    ...
    Panel.Expect("A strict non-half partial sector remains outside the half-turn route", PartialSource &&
                 !BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(PartialSource.Payload, ApexVertex, SetBack));
    ...
    Panel.Expect("A reflex sector remains outside the half-turn route", ReflexSource &&
                 !BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(ReflexSource.Payload, ApexVertex, SetBack));
    ...
    Panel.Expect("An unequal-radius half-turn bicone remains outside this equal route", UnequalSource &&
                 !BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(UnequalSource.Payload, ApexVertex, SetBack));
    ...
    Panel.Expect("A cylinder remains outside half-turn equal-apex dispatch", Cylinder &&
                 !BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(Cylinder.Payload, 0, SetBack));
    BrepBody Malformed = Source; Malformed.Edges[0].Coedges.push_back(Malformed.Edges[0].Coedges.front());
    Panel.Expect("A malformed half-turn source refuses transactionally", !BlendSolver::ClassifyHalfTurnEqualRadiusBiconeApexChamferVertex(Malformed, ApexVertex, SetBack));
    Panel.Expect("All refusal paths preserve the source", SameSource(Source, Snapshot, Before));
    Panel.Section("Distinct sharp-versus-equal-sector-chamfer proof");
    ...
    const bool Added = SourceDeliver && Result &&
        Host.Document().AddBody("HalfTurnEqualApexSource", Source.Transformed(Mat4::Translation({ -9, 0, 0 }))).Identity > 0 &&
        Host.Document().AddBody("HalfTurnEqualApexChamfer", Result.Payload.Transformed(Mat4::Translation({ 9, 0, 0 }))).Identity > 0;
    const bool Rendered = Added && Host.Execute("show shading flat") && Host.Execute("view iso") &&
        // Keep the camera outside the 105-degree sector so the translated proof bodies
        // avoid the shader's intentional red back-face diagnostic.
        Host.Execute("view orbit 300 -12") && Host.Execute("view fit") &&
        Host.Execute("render Phase39m_HalfTurnEqualRadiusBiconeApexChamfer");
    Panel.Expect("The half-turn equal-apex proof render completes", Rendered);
    Panel.Expect("The half-turn equal-apex proof PNG is visible", std::filesystem::exists(Proof) && std::filesystem::file_size(Proof, Error) > 100000);
