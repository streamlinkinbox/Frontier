//=============================================================================================================================================
// SolidArc · Phase 39c · bounded partial equal-radius bicone apex toroidal fillet
//=============================================================================================================================================
...
[[nodiscard]] bool OutputNormals(const BrepBody& Body, const PartialEqualRadiusBiconeApexFilletSpecification& S) noexcept
{
...
int main()
{
    VerificationPanel Panel("SolidArc · bounded partial equal-radius bicone apex toroidal fillet");
    const Fixture F{ { 0, 0, 0 }, { 0, 0, 1 }, 3.6, 3.6, 6.2, 4.7, 100.0 * ScalarCriteria::Pi / 180.0 };
    ...
    Panel.Expect("The source is a closed genus-zero equal-radius partial bicone", Before.Solid() && Before.Hulls == 1 && Before.Genus == 0 &&
    ...
    const auto Candidate = BlendSolver::ClassifyPartialEqualRadiusBiconeApexFilletVertex(Source, ApexVertex, FilletRadius);
    Panel.Expect("The partial equal-fillet dispatch accepts the selected vertex", static_cast<bool>(Candidate));
    if (Candidate)
    {
        ...
        Panel.Within("The native partial sweep is retained", std::fabs(Candidate.Payload.SweepAngle - F.Sweep), 1e-9);
        ...
        Panel.Expect("The equal partial dispatch is transactional", SameSource(Source, Snapshot, Before));
    }
    Panel.Section("Partial equal-radius toroidal reconstruction");
    const auto Result = Candidate ? BlendSolver::ReconstructPartialEqualRadiusBiconeApexFillet(Candidate.Payload)
                                  : Deliver<BrepBody>::Reject(RefusalReason::Unsupported, "no eligible equal partial apex");
    Panel.Expect("The partial equal toroidal fillet reconstructs one solid", Result && Result.Payload.Validate().Solid());
    if (Result)
    {
        ...
        Panel.Within("The partial volume is the sector-scaled equal cone/torus identity",
                     std::fabs(Report.Volume - FullVolume * F.Sweep / ScalarCriteria::TwoPi), 1e-1);
        Panel.Expect("The separate equal toroidal reconstruction preserves the source", SameSource(Source, Snapshot, Before));
    }
    Panel.Section("Strict partial equal-fillet refusal boundaries");
    Panel.Expect("A non-apex vertex refuses", !BlendSolver::ClassifyPartialEqualRadiusBiconeApexFilletVertex(Source, 0, FilletRadius));
    Panel.Expect("An out-of-range vertex refuses", !BlendSolver::ClassifyPartialEqualRadiusBiconeApexFilletVertex(Source, 99, FilletRadius));
    Panel.Expect("Zero, negative, and non-finite radii refuse", !BlendSolver::ClassifyPartialEqualRadiusBiconeApexFilletVertex(Source, ApexVertex, 0.0) &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeApexFilletVertex(Source, ApexVertex, -0.2) &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeApexFilletVertex(Source, ApexVertex, std::numeric_limits<double>::infinity()));
    if (Candidate)
    {
        ...
        Panel.Expect("A consuming fillet radius refuses", !BlendSolver::ReconstructPartialEqualRadiusBiconeApexFillet(TooLarge));
        Panel.Expect("A complete-turn specification refuses", !BlendSolver::ReconstructPartialEqualRadiusBiconeApexFillet(FullTurn));
    }
    ...
    Panel.Expect("An unequal-radius partial bicone remains outside this equal route", UnequalSource &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeApexFilletVertex(UnequalSource.Payload, ApexVertex, FilletRadius));
    ...
    Panel.Expect("A cylinder remains outside partial equal-apex dispatch", Cylinder &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeApexFilletVertex(Cylinder.Payload, 0, FilletRadius));
    BrepBody Malformed = Source; Malformed.Edges[0].Coedges.push_back(Malformed.Edges[0].Coedges.front());
    Panel.Expect("A malformed partial source refuses transactionally", !BlendSolver::ClassifyPartialEqualRadiusBiconeApexFilletVertex(Malformed, ApexVertex, FilletRadius));
    Panel.Expect("All refusal paths preserve the source", SameSource(Source, Snapshot, Before));
    Panel.Section("Distinct sharp-versus-partial-equal-toroidal proof");
    const std::filesystem::path Proof = std::filesystem::path(SOLIDARC_PROOF_FOLDER) / "Phase39c_PartialEqualBiconeApexFillet.png";
    std::error_code Error; std::filesystem::remove(Proof, Error);
    ConsoleHost Host(SOLIDARC_PROOF_FOLDER, 1600, 900);
    const bool Added = SourceDeliver && Result && Host.Document().AddBody("PartialEqualApexSource", Source.Transformed(Mat4::Translation({ -9, 0, 0 }))).Identity > 0 &&
        Host.Document().AddBody("PartialEqualApexToroidalFillet", Result.Payload.Transformed(Mat4::Translation({ 9, 0, 0 }))).Identity > 0;
    const bool Rendered = Added && Host.Execute("show shading flat") && Host.Execute("view iso") && Host.Execute("view orbit 300 -12") &&
        Host.Execute("view fit") && Host.Execute("render Phase39c_PartialEqualBiconeApexFillet");
    Panel.Expect("The partial equal-fillet proof render completes", Rendered);
    Panel.Expect("The partial equal-fillet proof PNG is visible", std::filesystem::exists(Proof) && std::filesystem::file_size(Proof, Error) > 100000);
