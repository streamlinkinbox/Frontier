//=============================================================================================================================================
// SolidArc · Phase 36j · bounded general-angle partial plane–cone root chamfer
//
...
[[nodiscard]] double AddedWedge(double FootRadius, double TopRadius, double BossHeight, double SetBack,
                                double SweepAngle) noexcept
{
    ...
    const double Full = ScalarCriteria::Pi * Axial / 3.0 *
...
int main()
{
    VerificationPanel Panel("SolidArc · Phase 36j · partial plane–cone root chamfer");
    ...
    Panel.Expect("The half-turn plane-cone chamfer commits exact V12/E17/F7 topology",
                 HalfChamfer && HalfApplied == 1 && HalfReport.Solid() && HalfReport.Hulls == 1 && HalfReport.Genus == 0 &&
                 HalfChamfer.Payload.Vertices.size() == 12 && HalfChamfer.Payload.Edges.size() == 17 &&
                 HalfChamfer.Payload.Coedges.size() == 34 && HalfChamfer.Payload.Loops.size() == 7 &&
                 HalfChamfer.Payload.Faces.size() == 7 && HalfReport.OpenEdges == 0 && HalfReport.NonManifoldEdges == 0 &&
    ...
    Panel.Expect("The partial plane-cone chamfer commits one complete chain", Result && Result.Payload.Validate().Solid() && Applied == 1);
    if (Result)
    {
        ...
        Panel.Within("The sector added wedge follows the exact angular square-radius integral",
                     std::fabs((Report.Volume - SourceReport.Volume) -
                               AddedWedge(FootRadius, TopRadius, BossHeight, SetBack, SweepAngle)) /
                         AddedWedge(FootRadius, TopRadius, BossHeight, SetBack, SweepAngle), 2e-3);
    ...
    const bool Added = Host.Document().AddBody("PartialPlaneConeSource", Source).Identity > 0;
    const bool ConsoleCommit = Added && !Members.empty() &&
        Host.Execute("chamfer PartialPlaneConeSource 0.5 --edges=" + std::to_string(Members.front()) + " --name=PartialPlaneConeChamfer") &&
        Host.Document().Find("PartialPlaneConeChamfer") &&
        Host.Document().Find("PartialPlaneConeChamfer")->Body.Validate().Solid();
    Panel.Expect("The console commits the bounded partial plane-cone route", ConsoleCommit);

    const std::filesystem::path Proof = std::filesystem::path(SOLIDARC_PROOF_FOLDER) / "Phase36j_PartialPlaneConeChamfer.png";
    ...
    const bool Rendered = SourceResult && ProofResult &&
        ProofHost.Document().AddBody("SharpPartialPlaneCone", Source.Transformed(Mat4::Translation({ -11, 0, 0 }))).Identity > 0 &&
        ProofHost.Document().AddBody("ChamferedPartialPlaneCone", ProofResult.Payload.Transformed(Mat4::Translation({ 11, 0, 0 }))).Identity > 0 &&
        ProofHost.Execute("view iso") && ProofHost.Execute("view fit") && ProofHost.Execute("render Phase36j_PartialPlaneConeChamfer");
