//=============================================================================================================================================
// SolidArc · Phase 39o · bounded reflex unequal-radius bicone apex chamfer
//=============================================================================================================================================
...
[[nodiscard]] Deliver<BrepBody> MakeSource(const Fixture& F) noexcept
{
    ...
    if (!Body || !CloseSector(Body.Payload, LowerBase, UpperBase, Radial, F.Sweep,
        ...
        return Deliver<BrepBody>::Reject(RefusalReason::NonManifold, "reflex source radial caps failed");
    return Body;
}
...
[[nodiscard]] bool OutputNormals(const BrepBody& Body, const ReflexUnequalRadiusBiconeApexChamferSpecification& S) noexcept
{
    ...
    const double LowerQ = S.LowerRadius * S.SetBack / S.LowerHeight;
    const double UpperQ = S.UpperRadius * S.SetBack / S.UpperHeight;
    ...
    for (size_t I = 0; I < Body.Faces.size(); ++I)
    {
        ...
        if (Surface.Classification == SurfaceClassification::Cone)
        {
            ...
            const bool Bridge = Surface.Origin.Distance(S.Apex - Axis * S.SetBack) <= 1e-9;
            const double Length = Lower ? S.LowerHeight - S.SetBack : (Bridge ? 2.0 * S.SetBack : S.UpperHeight - S.SetBack);
            const double Delta = Lower ? S.LowerRadius - LowerQ : (Bridge ? LowerQ - UpperQ : UpperQ - S.UpperRadius);
...
int main()
{
    VerificationPanel Panel("SolidArc · bounded reflex unequal-radius bicone apex chamfer");
    const Fixture F{ { 0, 0, 0 }, { 0, 0, 1 }, 4.7, 2.6, 6.3, 5.2, 4.0 * ScalarCriteria::Pi / 3.0 };
    const double SetBack = 0.8;
    const auto SourceDeliver = MakeSource(F);
    Panel.Expect("The distinct capped reflex source is constructed", static_cast<bool>(SourceDeliver));
    ...
    Panel.Expect("The source is a closed genus-zero reflex bicone", Before.Solid() && Before.Hulls == 1 &&
    ...
    const auto Candidate = BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, SetBack);
    Panel.Expect("The reflex unequal-apex dispatch accepts the selected vertex", static_cast<bool>(Candidate));
    if (Candidate)
    {
        ...
        Panel.Within("The exact reflex sweep is retained", std::fabs(S.SweepAngle - F.Sweep), 1e-9);
        Panel.Within("The axial set-back is retained", std::fabs(S.SetBack - SetBack), 1e-12);
        Panel.Expect("The radii remain unequal", std::fabs(S.LowerRadius - S.UpperRadius) > 1e-9);
        Panel.Expect("The reflex dispatch is deterministic and transactional",
                     BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, SetBack) &&
                     SameSource(Source, Snapshot, Before));
    }
    Panel.Section("Reflex three-frustum and radial-cap reconstruction");
    const auto Result = Candidate ? BlendSolver::ReconstructReflexUnequalRadiusBiconeApexChamfer(Candidate.Payload)
                                  : Deliver<BrepBody>::Reject(RefusalReason::Unsupported, "no eligible reflex apex");
    Panel.Expect("The reflex unequal chamfer reconstructs one solid", Result && Result.Payload.Validate().Solid());
    if (Result)
    {
        ...
        const double LowerQ = Candidate.Payload.LowerRadius * SetBack / Candidate.Payload.LowerHeight;
        const double UpperQ = Candidate.Payload.UpperRadius * SetBack / Candidate.Payload.UpperHeight;
        const double FullVolume = ScalarCriteria::Pi * (Candidate.Payload.LowerHeight - SetBack) *
            (Candidate.Payload.LowerRadius * Candidate.Payload.LowerRadius + Candidate.Payload.LowerRadius * LowerQ + LowerQ * LowerQ) / 3.0 +
            ScalarCriteria::Pi * 2.0 * SetBack * (LowerQ * LowerQ + LowerQ * UpperQ + UpperQ * UpperQ) / 3.0 +
        ...
        Panel.Within("The reflex volume is the exact sector-scaled three-frustum identity",
                     std::fabs(Report.Volume - FullVolume * F.Sweep / ScalarCriteria::TwoPi), 1e-1);
        Panel.Expect("The separate reflex reconstruction preserves the source", SameSource(Source, Snapshot, Before));
    }
    Panel.Section("Strict reflex unequal-apex refusal boundaries");
    Panel.Expect("A non-apex vertex refuses", !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(Source, 0, SetBack));
    Panel.Expect("An out-of-range vertex refuses", !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(Source, 99, SetBack));
    Panel.Expect("Zero, negative, and non-finite set-backs refuse",
                 !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, 0.0) &&
                 !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, -0.2) &&
                 !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, std::numeric_limits<double>::infinity()));
    Panel.Expect("A consuming set-back refuses", !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, F.UpperHeight));
    auto FullCandidate = Candidate.Payload; FullCandidate.SweepAngle = ScalarCriteria::TwoPi;
    Panel.Expect("A complete-turn specification refuses", !BlendSolver::ReconstructReflexUnequalRadiusBiconeApexChamfer(FullCandidate));
    ...
    Panel.Expect("A strict non-half partial sector remains outside the reflex route", PartialSource &&
                 !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(PartialSource.Payload, ApexVertex, SetBack));
    PartialFixture.Sweep = ScalarCriteria::Pi;
    const auto ReflexSource = MakeSource(PartialFixture);
    Panel.Expect("An exact half-turn sector remains outside the reflex route", ReflexSource &&
                 !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(ReflexSource.Payload, ApexVertex, SetBack));
    PartialFixture.Sweep = 5.0 * ScalarCriteria::Pi / 3.0;
    const auto OtherReflexSource = MakeSource(PartialFixture);
    Panel.Expect("A non-canonical reflex sector remains outside the reflex route", OtherReflexSource &&
                 !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(OtherReflexSource.Payload, ApexVertex, SetBack));
    ...
    Panel.Expect("An equal-radius partial bicone remains outside this unequal route", EqualSource &&
                 !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(EqualSource.Payload, ApexVertex, SetBack));
    const auto Cylinder = BrepBody::Cylinder(F.Apex - F.Axis * F.LowerHeight, F.Axis, F.LowerRadius, F.LowerHeight);
    Panel.Expect("A cylinder remains outside reflex unequal-apex dispatch", Cylinder &&
                 !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(Cylinder.Payload, 0, SetBack));
    BrepBody Malformed = Source; Malformed.Edges[0].Coedges.push_back(Malformed.Edges[0].Coedges.front());
    Panel.Expect("A malformed reflex source refuses transactionally", !BlendSolver::ClassifyReflexUnequalRadiusBiconeApexChamferVertex(Malformed, ApexVertex, SetBack));
    Panel.Expect("All refusal paths preserve the source", SameSource(Source, Snapshot, Before));
    Panel.Section("Distinct sharp-versus-sector-chamfer proof");
    ...
    const bool Added = SourceDeliver && Result &&
        Host.Document().AddBody("ReflexUnequalApexSource", Source.Transformed(Mat4::Translation({ -9, 0, 0 }))).Identity > 0 &&
        Host.Document().AddBody("ReflexUnequalApexChamfer", Result.Payload.Transformed(Mat4::Translation({ 9, 0, 0 }))).Identity > 0;
    const bool Rendered = Added && Host.Execute("show shading flat") && Host.Execute("view iso") &&
        // Keep the camera outside the reflex sector so the translated proof bodies
        // avoid the shader's intentional red back-face diagnostic.
        Host.Execute("view orbit 300 -12") && Host.Execute("view fit") &&
        Host.Execute("render Phase39o_ReflexUnequalRadiusBiconeApexChamfer");
    Panel.Expect("The reflex unequal-apex proof render completes", Rendered);
    Panel.Expect("The reflex unequal-apex proof PNG is visible", std::filesystem::exists(Proof) && std::filesystem::file_size(Proof, Error) > 100000);
