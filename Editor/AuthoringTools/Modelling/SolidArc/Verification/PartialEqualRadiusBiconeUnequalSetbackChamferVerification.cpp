//=============================================================================================================================================
// SolidArc · Phase 39g · bounded partial equal-radius bicone chamfer with independent set-backs
//=============================================================================================================================================
...
[[nodiscard]] bool OutputNormals(
    const BrepBody& Body, const PartialEqualRadiusBiconeUnequalSetbackChamferSpecification& S) noexcept
{
...
int main()
{
    VerificationPanel Panel("SolidArc · bounded partial equal-radius bicone unequal-setback chamfer");
    const Fixture F{ { 0, 0, 0 }, { 0, 0, 1 }, 3.7, 3.7, 6.4, 4.8, 115.0 * ScalarCriteria::Pi / 180.0 };
    const double LowerSetBack = 0.65;
    const double UpperSetBack = 1.05;
    ...
    const auto Candidate = BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, LowerSetBack, UpperSetBack);
    Panel.Expect("The partial independent-setback dispatch accepts the selected vertex", static_cast<bool>(Candidate));
    if (Candidate)
    {
        ...
        Panel.Expect("The partial independent-setback dispatch is deterministic and transactional",
    ...
    const auto Result = Candidate ? BlendSolver::ReconstructPartialEqualRadiusBiconeUnequalSetbackChamfer(Candidate.Payload)
                                  : Deliver<BrepBody>::Reject(RefusalReason::Unsupported, "no eligible partial apex");
    Panel.Expect("The partial independent-setback chamfer reconstructs one solid", Result && Result.Payload.Validate().Solid());
    if (Result)
    {
        ...
        const double LowerQ = Candidate.Payload.Radius * LowerSetBack / Candidate.Payload.LowerHeight;
        const double UpperQ = Candidate.Payload.Radius * UpperSetBack / Candidate.Payload.UpperHeight;
    ...
    Panel.Section("Strict partial independent-setback refusal boundaries");
    Panel.Expect("A non-apex vertex refuses", !BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(
        Source, 0, LowerSetBack, UpperSetBack));
    Panel.Expect("An out-of-range vertex refuses", !BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(
        Source, 99, LowerSetBack, UpperSetBack));
    Panel.Expect("Zero, negative, and non-finite set-backs refuse",
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, 0.0, UpperSetBack) &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, LowerSetBack, -0.2) &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, LowerSetBack,
                                                                                         std::numeric_limits<double>::infinity()));
    Panel.Expect("Equal set-backs remain on the single-setback route",
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, 0.9, 0.9));
    Panel.Expect("A consuming lower or upper set-back refuses",
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, F.LowerHeight, UpperSetBack) &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, LowerSetBack, F.UpperHeight));
    auto FullCandidate = Candidate.Payload; FullCandidate.SweepAngle = ScalarCriteria::TwoPi;
    Panel.Expect("A complete-turn specification refuses", !BlendSolver::ReconstructPartialEqualRadiusBiconeUnequalSetbackChamfer(FullCandidate));
    const Fixture UnequalFixture{ F.Apex, F.Axis, 4.6, 2.5, F.LowerHeight, F.UpperHeight, F.Sweep };
    ...
    Panel.Expect("An unequal-radius partial bicone remains outside this equal route", UnequalSource &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(UnequalSource.Payload, ApexVertex,
    ...
    Panel.Expect("A cylinder remains outside partial independent-setback dispatch", Cylinder &&
                 !BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(Cylinder.Payload, 0,
                                                                                         LowerSetBack, UpperSetBack));
    BrepBody Malformed = Source; Malformed.Edges[0].Coedges.push_back(Malformed.Edges[0].Coedges.front());
    Panel.Expect("A malformed partial source refuses transactionally", !BlendSolver::ClassifyPartialEqualRadiusBiconeUnequalSetbackChamferVertex(
    ...
    const bool Added = SourceDeliver && Result &&
        Host.Document().AddBody("PartialEqualSetbackApexSource", Source.Transformed(Mat4::Translation({ -9, 0, 0 }))).Identity > 0 &&
        Host.Document().AddBody("PartialEqualSetbackApexChamfer", Result.Payload.Transformed(Mat4::Translation({ 9, 0, 0 }))).Identity > 0;
    const bool Rendered = Added && Host.Execute("show shading flat") && Host.Execute("view iso") &&
        ...
        Host.Execute("render Phase39g_PartialEqualBiconeUnequalSetbackChamfer");
    Panel.Expect("The partial independent-setback proof render completes", Rendered);
    Panel.Expect("The partial independent-setback proof PNG is visible", std::filesystem::exists(Proof) && std::filesystem::file_size(Proof, Error) > 100000);
