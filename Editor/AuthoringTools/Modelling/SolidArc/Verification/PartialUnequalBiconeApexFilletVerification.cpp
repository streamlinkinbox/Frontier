...
[[nodiscard]] bool OutputNormals(const BrepBody& Body, const PartialUnequalBiconeApexFilletSpecification& S) noexcept
{
...
int main()
{
    VerificationPanel Panel("SolidArc · bounded partial unequal-radius bicone apex toroidal fillet");
    const Fixture F{ { 0, 0, 0 }, { 0, 0, 1 }, 4.5, 2.7, 6.8, 4.6, 95.0 * ScalarCriteria::Pi / 180.0 };
    ...
    Panel.Expect("The source is a closed genus-zero partial bicone", Before.Solid() && Before.Hulls == 1 && Before.Genus == 0 &&
    ...
    const auto Candidate = BlendSolver::ClassifyPartialUnequalBiconeApexFilletVertex(Source, ApexVertex, FilletRadius);
    Panel.Expect("The partial unequal-fillet dispatch accepts the selected vertex", static_cast<bool>(Candidate));
    if (Candidate)
    {
        ...
        Panel.Expect("The partial-fillet dispatch is transactional", SameSource(Source, Snapshot, Before));
    }
    Panel.Section("Partial toroidal reconstruction and tangent contacts");
    const auto Result = Candidate ? BlendSolver::ReconstructPartialUnequalBiconeApexFillet(Candidate.Payload)
                                  : Deliver<BrepBody>::Reject(RefusalReason::Unsupported, "no eligible partial apex");
    Panel.Expect("The partial unequal toroidal fillet reconstructs one solid", Result && Result.Payload.Validate().Solid());
    if (Result)
    {
        ...
        Panel.Within("The partial volume is the sector-scaled cone/torus identity",
    ...
    Panel.Section("Strict partial unequal-fillet refusal boundaries");
    Panel.Expect("A non-apex vertex refuses", !BlendSolver::ClassifyPartialUnequalBiconeApexFilletVertex(Source, 0, FilletRadius));
    Panel.Expect("An out-of-range vertex refuses", !BlendSolver::ClassifyPartialUnequalBiconeApexFilletVertex(Source, 99, FilletRadius));
    Panel.Expect("Zero, negative, and non-finite radii refuse",
                 !BlendSolver::ClassifyPartialUnequalBiconeApexFilletVertex(Source, ApexVertex, 0.0) &&
                 !BlendSolver::ClassifyPartialUnequalBiconeApexFilletVertex(Source, ApexVertex, -0.2) &&
                 !BlendSolver::ClassifyPartialUnequalBiconeApexFilletVertex(Source, ApexVertex, std::numeric_limits<double>::infinity()));
    if (Candidate)
    {
        ...
        Panel.Expect("A consuming fillet radius refuses", !BlendSolver::ReconstructPartialUnequalBiconeApexFillet(TooLarge));
        Panel.Expect("A complete-turn specification refuses", !BlendSolver::ReconstructPartialUnequalBiconeApexFillet(FullTurn));
    }
    ...
    Panel.Expect("An equal-radius partial bicone remains outside this unequal route", EqualSource &&
                 !BlendSolver::ClassifyPartialUnequalBiconeApexFilletVertex(EqualSource.Payload, ApexVertex, FilletRadius));
    ...
    Panel.Expect("A cylinder remains outside partial unequal-apex dispatch", Cylinder &&
                 !BlendSolver::ClassifyPartialUnequalBiconeApexFilletVertex(Cylinder.Payload, 0, FilletRadius));
    BrepBody Malformed = Source; Malformed.Edges[0].Coedges.push_back(Malformed.Edges[0].Coedges.front());
    Panel.Expect("A malformed partial source refuses transactionally", !BlendSolver::ClassifyPartialUnequalBiconeApexFilletVertex(Malformed, ApexVertex, FilletRadius));
    Panel.Expect("All refusal paths preserve the source", SameSource(Source, Snapshot, Before));
    Panel.Section("Distinct sharp-versus-partial-toroidal proof");
    const std::filesystem::path Proof = std::filesystem::path(SOLIDARC_PROOF_FOLDER) / "Phase39b_PartialUnequalBiconeApexFillet.png";
    ...
    const bool Added = SourceDeliver && Result &&
        ...
        Host.Document().AddBody("PartialUnequalApexToroidalFillet", Result.Payload.Transformed(Mat4::Translation({ 9, 0, 0 }))).Identity > 0;
    const bool Rendered = Added && Host.Execute("show shading flat") && Host.Execute("view iso") &&
        ...
        Host.Execute("render Phase39b_PartialUnequalBiconeApexFillet");
    Panel.Expect("The partial unequal-fillet proof render completes", Rendered);
    Panel.Expect("The partial unequal-fillet proof PNG is visible", std::filesystem::exists(Proof) && std::filesystem::file_size(Proof, Error) > 100000);
