//=============================================================================================================================================
// SolidArc · Phase 39j · bounded half-turn unequal-radius bicone chamfer with independent set-backs
//=============================================================================================================================================
...
[[nodiscard]] bool OutwardNormals(
    const BrepBody& Body, const HalfTurnUnequalRadiusBiconeUnequalSetbackChamferSpecification& S) noexcept
{
...
[[nodiscard]] bool ExactSurfaces(const BrepBody& Body,
                                 const HalfTurnUnequalRadiusBiconeUnequalSetbackChamferSpecification& S) noexcept
{
    const Vec3 Axis = S.Axis.Normalised();
...
int main()
{
    VerificationPanel Panel("SolidArc · bounded half-turn unequal-radius bicone chamfer with independent set-backs");
    const Fixture F{ { 0, 0, 0 }, { 0, 0, 1 }, 4.8, 2.6, 6.7, 4.3, ScalarCriteria::Pi };
    const double LowerSetBack = 0.75;
    const double UpperSetBack = 1.15;
    const Deliver<BrepBody> SourceDeliver = MakePartialBicone(F);
    Panel.Expect("The distinct half-turn unequal-radius independent-setback source is constructed", static_cast<bool>(SourceDeliver));
    ...
    Panel.Expect("The source is a closed genus-zero half-turn unequal-radius bicone", Before.Solid() && Before.Hulls == 1 &&
    ...
    const Deliver<HalfTurnUnequalRadiusBiconeUnequalSetbackChamferSpecification> Candidate =
        BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex,
                                                                          LowerSetBack, UpperSetBack);
    Panel.Expect("The half-turn unequal-radius independent-setback apex dispatch accepts the selected vertex", static_cast<bool>(Candidate));
    if (Candidate)
    {
        const auto& S = Candidate.Payload;
        Panel.Within("The lower support radius is extracted exactly", std::fabs(S.LowerRadius - F.LowerRadius), 1e-12);
        Panel.Within("The upper support radius is extracted exactly", std::fabs(S.UpperRadius - F.UpperRadius), 1e-12);
        ...
        Panel.Expect("The half-turn unequal-radius independent-setback classification is transactional", SameSource(Source, Snapshot, Before));
        const auto Repeat = BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(
            Source, ApexVertex, LowerSetBack, UpperSetBack);
        Panel.Expect("The half-turn unequal-radius independent-setback dispatch is deterministic", Repeat &&
    ...
    const auto Result = Candidate ? BlendSolver::ReconstructHalfTurnUnequalRadiusBiconeUnequalSetbackChamfer(Candidate.Payload)
                                  : Deliver<BrepBody>::Reject(RefusalReason::Unsupported, "no eligible apex");
    Panel.Expect("The half-turn unequal-radius independent-setback reconstruction is a solid", Result && Result.Payload.Validate().Solid());
    if (Result)
    {
    ...
    Panel.Section("Half-turn unequal-radius independent-setback refusal boundaries");
    Panel.Expect("A non-apex vertex refuses", !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(
        Source, 0, LowerSetBack, UpperSetBack));
    Panel.Expect("An out-of-range vertex refuses", !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(
        Source, 99, LowerSetBack, UpperSetBack));
    Panel.Expect("Equal set-backs remain on the existing equal-setback routes", !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(
        Source, ApexVertex, 1.0, 1.0));
    Panel.Expect("Zero, negative, and non-finite set-backs refuse",
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, 0.0, UpperSetBack) &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, LowerSetBack, -0.2) &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, LowerSetBack,
                                                                                   std::numeric_limits<double>::infinity()));
    Panel.Expect("A consuming lower or upper set-back refuses",
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, F.LowerHeight, UpperSetBack) &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(Source, ApexVertex, LowerSetBack, F.UpperHeight));
    const Fixture EqualRadius{ F.Apex, F.Axis, 3.7, 3.7, F.LowerHeight, F.UpperHeight, ScalarCriteria::Pi };
    const auto EqualSource = MakePartialBicone(EqualRadius);
    Panel.Expect("An equal-radius source remains outside the half-turn unequal route", EqualSource &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(EqualSource.Payload, ApexVertex,
                                                                                    LowerSetBack, UpperSetBack));
    ...
    Panel.Expect("A complete-turn source remains outside the half-turn route", FullSource &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(FullSource.Payload, 1,
                                                                                            LowerSetBack, UpperSetBack));
    ...
    Panel.Expect("A strict non-half partial sector remains outside the half-turn route", PartialSource &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(PartialSource.Payload, 2,
                                                                                            LowerSetBack, UpperSetBack));
    ...
    Panel.Expect("A reflex sector remains outside the half-turn route", ReflexSource &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(ReflexSource.Payload, 2,
                                                                                            LowerSetBack, UpperSetBack));
    ...
    Panel.Expect("A cylinder remains outside half-turn independent-setback dispatch", Cylinder &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(Cylinder.Payload, 0,
                                                                                            LowerSetBack, UpperSetBack));
    ...
    Panel.Expect("A freeform sphere remains outside half-turn bicone dispatch", Sphere &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(Sphere.Payload, 0,
                                                                                            LowerSetBack, UpperSetBack));
    BrepBody Malformed = Source;
    Malformed.Edges[0].Coedges.push_back(Malformed.Edges[0].Coedges.front());
    Panel.Expect("A malformed source refuses transactionally", !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeUnequalSetbackChamferVertex(
    ...
    const bool Added = SourceDeliver && Result &&
        Host.Document().AddBody("UnequalRadiusIndependentSetbackSource", Source.Transformed(Mat4::Translation({ -9, 0, 0 }))).Identity > 0 &&
        Host.Document().AddBody("UnequalRadiusIndependentSetbackChamfer", Result.Payload.Transformed(Mat4::Translation({ 9, 0, 0 }))).Identity > 0;
    const bool Rendered = Added && Host.Execute("show shading flat") && Host.Execute("view iso") &&
        ...
        Host.Execute("render Phase39j_HalfTurnUnequalRadiusBiconeUnequalSetbackChamfer");
    Panel.Expect("The half-turn unequal-radius independent-setback proof render completes", Rendered);
    Panel.Expect("The half-turn unequal-radius independent-setback proof PNG is visible",
