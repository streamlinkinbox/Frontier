//=============================================================================================================================================
// SolidArc · Phase 39n · bounded half-turn unequal-radius bicone apex chamfer
//=============================================================================================================================================
...
[[nodiscard]] bool OutputNormals(const BrepBody& Body, const HalfTurnUnequalRadiusBiconeApexChamferSpecification& S) noexcept
{
...
int main()
{
    VerificationPanel Panel("SolidArc · bounded half-turn unequal-radius bicone apex chamfer");
    const Fixture F{ { 0, 0, 0 }, { 0, 0, 1 }, 4.2, 2.8, 6.4, 4.8, ScalarCriteria::Pi };
    const double SetBack = 0.9;
    ...
    Panel.Expect("The source is a closed genus-zero half-turn bicone", Before.Solid() && Before.Hulls == 1 &&
    ...
    const auto Candidate = BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, SetBack);
    Panel.Expect("The half-turn unequal-apex dispatch accepts the selected vertex", static_cast<bool>(Candidate));
    if (Candidate)
    {
        ...
        Panel.Expect("The half-turn dispatch is deterministic and transactional",
                     BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, SetBack) &&
                     SameSource(Source, Snapshot, Before));
    }
    Panel.Section("Half-turn three-frustum and radial-cap reconstruction");
    const auto Result = Candidate ? BlendSolver::ReconstructHalfTurnUnequalRadiusBiconeApexChamfer(Candidate.Payload)
                                  : Deliver<BrepBody>::Reject(RefusalReason::Unsupported, "no eligible half-turn apex");
    Panel.Expect("The half-turn unequal chamfer reconstructs one solid", Result && Result.Payload.Validate().Solid());
    if (Result)
    {
    ...
    Panel.Section("Strict half-turn unequal-apex refusal boundaries");
    Panel.Expect("A non-apex vertex refuses", !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(Source, 0, SetBack));
    Panel.Expect("An out-of-range vertex refuses", !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(Source, 99, SetBack));
    Panel.Expect("Zero, negative, and non-finite set-backs refuse",
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, 0.0) &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, -0.2) &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, std::numeric_limits<double>::infinity()));
    Panel.Expect("A consuming set-back refuses", !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(Source, ApexVertex, F.UpperHeight));
    auto FullCandidate = Candidate.Payload; FullCandidate.SweepAngle = ScalarCriteria::TwoPi;
    Panel.Expect("A complete-turn specification refuses", !BlendSolver::ReconstructHalfTurnUnequalRadiusBiconeApexChamfer(FullCandidate));
    ...
    Panel.Expect("A strict non-half partial sector remains outside the half-turn route", PartialSource &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(PartialSource.Payload, ApexVertex, SetBack));
    ...
    Panel.Expect("A reflex sector remains outside the half-turn route", ReflexSource &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(ReflexSource.Payload, ApexVertex, SetBack));
    ...
    Panel.Expect("An equal-radius partial bicone remains outside this unequal route", EqualSource &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(EqualSource.Payload, ApexVertex, SetBack));
    ...
    Panel.Expect("A cylinder remains outside half-turn unequal-apex dispatch", Cylinder &&
                 !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(Cylinder.Payload, 0, SetBack));
    BrepBody Malformed = Source; Malformed.Edges[0].Coedges.push_back(Malformed.Edges[0].Coedges.front());
    Panel.Expect("A malformed half-turn source refuses transactionally", !BlendSolver::ClassifyHalfTurnUnequalRadiusBiconeApexChamferVertex(Malformed, ApexVertex, SetBack));
    ...
    const bool Added = SourceDeliver && Result &&
        ...
        Host.Document().AddBody("HalfTurnUnequalApexChamfer", Result.Payload.Transformed(Mat4::Translation({ 9, 0, 0 }))).Identity > 0;
    const bool Rendered = Added && Host.Execute("show shading flat") && Host.Execute("view iso") &&
        ...
        Host.Execute("render Phase39n_HalfTurnUnequalRadiusBiconeApexChamfer");
    Panel.Expect("The half-turn unequal-apex proof render completes", Rendered);
    Panel.Expect("The half-turn unequal-apex proof PNG is visible", std::filesystem::exists(Proof) && std::filesystem::file_size(Proof, Error) > 100000);
