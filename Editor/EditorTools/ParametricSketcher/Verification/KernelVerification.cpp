...
int main()
{
    ...
    Panel.Section("BlendSolver · asymmetric endpoint support classifier");
    {
        ...
        Deliver<BrepBody> Translated = BlendSolver::ReconstructAsymmetricFrustum(Transformed);
        Panel.Expect("Translated oblique asymmetric frustum reconstructs", Translated && Translated.Payload.Validate().Solid());
        const double FrustumExact = ScalarCriteria::Pi * 8.0 / 3.0 * (4.0 + 2.5 + 1.5625);
        const double FrustumMeasured = Frustum ? Frustum.Payload.Validate().Volume : 0.0;
        Panel.Expect("Asymmetric frustum volume passes the centralized gate",
                     Frustum && ScalarCriteria::WithinVolumeTolerance(FrustumMeasured, FrustumExact));
        Panel.Expect("Asymmetric frustum volume is analytic at the tightened band",
                     Frustum && ScalarCriteria::WithinMeasuredBand(
                         Frustum.Payload.Validate(ScalarCriteria::VerificationChord, ScalarCriteria::VerificationCap).Volume,
                         FrustumExact));
        ...
        Deliver<BrepBody> UnequalRadial = BlendSolver::ReconstructAsymmetricSupport(Taper);
        Panel.Expect("Unequal radial caps reconstruct through the bounded support route",
                     UnequalRadial && UnequalRadial.Payload.Validate().Solid());
        Taper.Kind = AsymmetricSupportKind::PartialEndpointChain;
        Deliver<BrepBody> PartialChain = BlendSolver::ReconstructAsymmetricSupport(Taper);
        Panel.Expect("Partial endpoint chain remains explicitly refused", !PartialChain);
        Panel.Expect("Partial endpoint chain refusal reason is Unsupported",
        ...
        Deliver<BrepBody> EqualPlanes = BlendSolver::ReconstructAsymmetricSupport(Taper);
        Panel.Expect("Equal-radius asymmetric planes reconstruct", EqualPlanes && EqualPlanes.Payload.Validate().Solid());
    }
