//============================================================================================================================================
// 📦 Editor/EditorTools/ParametricSketcher/Verification/FaceLoftVerification.cpp — Phase 34a face loft between two solids
//============================================================================================================================================
// Two closed solids become one by skinning between a chosen face of each: the faces are dropped, their rims become the
// sections of a ruled loft, and the skin is sewn to the survivors along the very edges the dropped faces used. There is
// no Boolean anywhere in the route, so the shared rims are exact; the analytic cases (box→box prism, coaxial cap→cap
// frustum) pin the volumes, box→cylinder pins the topology, seam split and least-twist ruling, and every unsupported
// selection refuses with the sources untouched.
...
// Index of the face whose outward normal (sampled at its tessellation centroid) points along Direction.
[[nodiscard]] int FaceToward(const BrepBody& Body, Vec3 Direction) noexcept
{
    ...
    for (int F = 0; F < static_cast<int>(Body.Faces.size()); ++F)
    {
        ...
        const double D = Area.Normalised().Dot(Direction.Normalised());
...
int main()
{
    ...
    Panel.Expect("Two faces of one body refuse", !SkinSolver::LoftFaces(A, APlusX, A, FaceToward(A, { -1, 0, 0 })));
    {
    ...
    Panel.Expect("A face token on a curve or a single token refuses at the console",
                 !CommandHost.Execute("loft Nose:f0") && !CommandHost.Execute("loft Nose:f0 Nose:f1"));
