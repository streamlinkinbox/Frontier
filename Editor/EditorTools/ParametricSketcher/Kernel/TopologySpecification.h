...
public:
    ...
    // Volume and area are divergence-theorem integrals over the face tessellations, so the measured value carries the
    //    tessellation's own discretisation error. Callers that compare against a closed form may pass a refined chord:
    //    the default band is the shipping gate (ScalarCriteria::VolumeTolerance), a refined chord proves the analytic
    //    claim at a much tighter band without changing what the kernel accepts.
    [[nodiscard]] BodyReport  Validate(double Chord = ScalarCriteria::ChordTolerance, int MaximumPerSpan = ScalarCriteria::ChordSubdivision) const noexcept;
    [[nodiscard]] Box3        Bounds() const noexcept;
    [[nodiscard]] Vec3        FaceNormal(int Face, double U, double V) const noexcept;  // honours Reversed
    [[nodiscard]] double      SignedVolume(double Chord = ScalarCriteria::ChordTolerance, int MaximumPerSpan = ScalarCriteria::ChordSubdivision) const noexcept;  // divergence theorem over the face tessellations
    [[nodiscard]] double      Area(double Chord = ScalarCriteria::ChordTolerance, int MaximumPerSpan = ScalarCriteria::ChordSubdivision) const noexcept;
    ...
    [[nodiscard]] FaceTriangles TessellateFace(int Face, double ChordTolerance = ScalarCriteria::ChordTolerance,
                                              int MaximumPerSpan = ScalarCriteria::ChordSubdivision) const noexcept;
