...
BrepBody::FaceTriangles BrepBody::TessellateFace(int Face, double ChordTolerance, int MaximumPerSpan) const noexcept
{
    FaceTriangles Out;
    const BrepFace& F = Faces[Face];
    if (F.Natural)
    {
        NurbsSurface::Tessellation T = F.Surface.Tessellate(ChordTolerance, 2, MaximumPerSpan);
    ...
    else
    {
        ...
        {
            NurbsSurface::Tessellation T = S.Tessellate(ChordTolerance, 2, MaximumPerSpan);
...
double BrepBody::SignedVolume(double Chord, int MaximumPerSpan) const noexcept
{
    double V = 0;
    for (size_t F = 0; F < Faces.size(); ++F)
    {
        FaceTriangles T = TessellateFace(static_cast<int>(F), Chord, MaximumPerSpan);
...
double BrepBody::Area(double Chord, int MaximumPerSpan) const noexcept
{
    double A = 0;
    for (size_t F = 0; F < Faces.size(); ++F)
    {
...
BodyReport BrepBody::Validate(double Chord, int MaximumPerSpan) const noexcept
{
    BodyReport R;
    ...
    R.Volume = R.Closed ? SignedVolume(Chord, MaximumPerSpan) : 0.0;                                    // divergence theorem needs a closed boundary
    R.Area = Area(Chord, MaximumPerSpan);
    return R;
}
