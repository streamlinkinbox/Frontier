                                                           ...
                                                           double MinimumClearance) noexcept
{
    ...
    for (size_t I = 0; I + 1 < Chain.Supports.size(); ++I)
    {
        const EndpointSupport& Low = Chain.Supports[I];
        const EndpointSupport& High = Chain.Supports[I + 1];
        const double Height = (High.Centre - Low.Centre).Length();
        Deliver<NurbsSurface> Span = NurbsSurface::Cone(Low.Centre, Axis, Low.Radius, High.Radius, Height);
        if (!Span) return Deliver<BrepBody>::Reject(Span.Denial.Reason, "chain span does not build");
        Spans.push_back(std::move(Span.Payload));
        ExactVolume += ScalarCriteria::Pi * Height *
                       (Low.Radius * Low.Radius + Low.Radius * High.Radius + High.Radius * High.Radius) / 3.0;
    }
    Deliver<BrepBody> Result = BrepBody::Sew(Spans);
    if (!Result) return Deliver<BrepBody>::Reject(Result.Denial.Reason, "chain spans do not sew");
    const BodyReport Report = Result.Payload.Validate();
    if (!Report.Solid() || Report.Hulls != 1)
        return Deliver<BrepBody>::Reject(RefusalReason::NonManifold, "chain does not close into one manifold solid");
    if (!ScalarCriteria::WithinVolumeTolerance(Report.Volume, ExactVolume))
        return Deliver<BrepBody>::Reject(RefusalReason::Unsupported, "chain volume failed analytic acceptance");
    return Result;
}
