...
public:
    ...
    // Finite-support network: consecutive orthogonal supports walking one axis, each span a conical ruled surface between
    //    two parallel circular rims, sewn into one closed solid. Every span is convex between its own rims, so a chain
    //    whose support positions strictly increase cannot self-intersect — the monotonicity classification *is* the
    //    closed-manifold guarantee. An open, folding, oblique, or non-positive chain refuses; nothing is approximated.
    [[nodiscard]] static Deliver<BrepBody> ReconstructAsymmetricChain(const AsymmetricEndpointChain& Chain,
