...
// npm install --prefix .cache/native-icon-browser playwright-core@1.63.0 @sparticuz/chromium@153.0.0
...
const {default:chromium}=await import(path.join(root,'.cache/native-icon-browser/node_modules/@sparticuz/chromium/build/index.js'));
const {chromium:playwright}=await import(path.join(root,'.cache/native-icon-browser/node_modules/playwright-core/index.mjs'));
// Use the package's bundled NSS/NSPR libraries on minimal Linux build workers.
const libs=path.join(root,'.cache/native-icon-browser/libs');await mkdir(libs,{recursive:true});
const tar=path.join(root,'.cache/native-icon-browser/libs.tar');
await writeFile(tar,brotliDecompressSync(await readFile(path.join(root,'.cache/native-icon-browser/node_modules/@sparticuz/chromium/bin/al2023.tar.br'))));
...
try{
 for(const name of ['sun','moon']){
  ...
  const authority=await readFile(path.join(root,'Experimental/FrontierEditor/custom-icons',name+'.svg'));
