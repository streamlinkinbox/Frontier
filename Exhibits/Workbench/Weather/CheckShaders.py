#!/usr/bin/env python3
"""Compile the unchanged GPU shader table as a regression gate, not a GPU weather execution proof."""
...
t=R/'.cache/cpp-sun-full';out=R/'Exhibits/Gallery/WeatherNative';stage=R/'.cache/weather-shader-output';env=dict(os.environ,PATH=str(compiler.parent)+os.pathsep+os.environ['PATH'],SHADER_OUT=str(stage));command=['bash',str(t/'Tools/Build/CheckShaders.sh')]
result=subprocess.run(command,cwd=t,env=env,text=True,stdout=subprocess.PIPE,stderr=subprocess.STDOUT);(out/'ShaderCheck.txt').write_text(result.stdout+'\nRegression compilation only. Wind, precipitation and rainbow device execution is not verified. No GPU execution claim.\n');print(result.stdout);assert result.returncode==0 and 'GREEN — 18/18' in result.stdout
