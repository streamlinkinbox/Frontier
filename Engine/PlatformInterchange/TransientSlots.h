...
//                                                        TRANSIENTSLOTS.H
//============================================================================================================================================
// 🧩 One-shot voices spawned by throttle events — lift-off backfire thud, overrun pop, anti-lag pop, blow-off flutter —
//    ported from the AudioEditor's `TransientSlots` (Tools/AudioEditor/index.html, <script id="dsp">). Fixed capacity, no
//    allocation: a spawn that finds no free slot is counted as dropped (the editor's `dropped` readout; the proofs hold it at 0
//    on the limiter sequence with 32 slots). Spawn takes the editor's nine arguments in the editor's order so the call sites
//    in AcousticIntegrator read the same as the JavaScript, and Advance draws its noise from the caller's Xorshift in the
//    same statement order (that order is part of the ±1 × 10⁻⁶ identity contract).
...
enum class TransientCategory : uint8_t
{
    ...
    Blowoff  = 3    // high-passed noise sweeping 3 kHz → 1 kHz with a 14 Hz flutter
};
...
public:
    ...
    // (type, delay [s], duration [s], f0 [Hz], f1 [Hz], gain, attack [s], noise gain, one-pole corner [Hz]) — the editor's order.
    void Spawn(TransientCategory Type, double Delay, double Dur, double F0, double F1, double Gain, double Attack, double NoiseGain, double LowHz) noexcept
    {
        int32_t Slot = -1;
    ...
    [[nodiscard]] double Advance(Xorshift& Rng) noexcept
    {
        ...
        for (uint32_t I = 0u; I < Capacity; ++I)
        {
            ...
            if (Type == TransientCategory::Blowoff)
            {
                ...
                const double Flutter = 0.5 + 0.5 * std::sin(TwoPi * 14.0 * Time);
                Out += High[I].Advance(Rng.Signed()) * Level[I] * Env * Flutter;
            }
            else
            {
                const double Hz = Start[I] * std::pow(End[I] / Start[I], Frac);
