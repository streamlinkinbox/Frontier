...
- `CelestialCore.slang` — the panel's sky + sun as pure functions
  ...
  `celSunGround`, `celSkyField`, `celSkyFull`, `celStarField`, `celStarAA`,
  `celApplyMedia`, `celSkyAmbient`, `celSkyPixel`, `celApplyPost`,
  viewport rays).
- `CelestialReSTIR.slang` — deterministic RNG, candidate draws, DI/GI
  ...
  9 compute entries (`skyProbe`, `diInitial`, `diTemporal`, `diSpatial`,
  `giInitial`, `giTemporal`, `giSpatial`, `shade`, `skyViewport`).
...
- **DI candidates**: deterministic mixture over sun-cone ×2 / aureole-lobe /
  sphere. Every candidate evaluates THE sky (`celSkyFull`: disc + aureole +
  atmosphere + glow + stars + media) and is weighted by the balance heuristic
  against the mixture pdf
...
|---|---|---|
| b0 `CelFrame` | 18×float4 | sun dir/color+elev, sun params, atmosphere A/B, sky tint, ground, post A/B, misc, media A-E, stars A-C |
| b1 `CelLaunch` | 8×uint | width, height, frame, M0, Mcap, spatial taps, seed, pad |
| b2 `CelCam` | 4×float4 | world-frame camera basis + tanHalf (all entries: star pixel-angle) |
...
| u3 | float3[1] | sky-ambient probe (written by `skyProbe`, read by all entries) |

Per frame the host: fills b0/b1/b2, dispatches `skyProbe` (1 thread,
barrier on u3), then
