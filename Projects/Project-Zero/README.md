...
- `Shaders/` — the shipped code: `CelestialCore.slang` (sky + sun),
  `CelestialReSTIR.slang` (reservoirs + the 8 compute entries). See
...
- `Host/` — CPU harness that compiles the shipped `.slang` verbatim as C++17:
  ...
  `ReSTIRConvergence` (T2/T3/T4), `SunPosition.h` (host ephemeris),
  `SlangCompat.h` (dual-compile prelude), `CpuPort/` (vendored upstream
...
```sh
cd Host && make            # SkyViewport CpuPortDiff ReSTIRConvergence
cd ../Diagnostics && python3 RunCelestialParity.py   # everything (about 1 min)
```

`RunCelestialParity.py` renders the 6-frame matrix (dawn/day/sun-disc), checks
it against the oracle and the upstream port (max 1 LSB), runs the static
rules, and runs the ReSTIR convergence proof. Exit code 0 iff all green;
...
- Parity: 6/6 frames vs the oracle at max 1 LSB full-frame (ground and sun
  disc included); 6/6 vs the upstream port at max 1 LSB on the sky mask.
...
- Background: the review that specified this work is
  `Reviews/SunSky-Parity-2026-09-14.md`.
