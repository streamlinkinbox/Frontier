        ...
        for (uint32_t x = 0; x < kW; ++x)
        {
            ...
            if (g.depth > 0.0f)
            {
                ...
                for (uint i = 0u; i < kM0; i++)
                {
                    ...
                    rng = d.rng;
                    const CelAtmo f = celSkyField(d.dir, p);
                    float3 skyUp = f.sky;
                    if (!(d.dir.y > 0.0f))
                    {
                        skyUp = float3(0.0f, 0.0f, 0.0f);
                    }
                    const float3 leFull = celSunDisc(d.dir, f.trans, p)
                                        + celSunAureole(d.dir, f.trans, p) + skyUp;
...
BruteRef BruteForceDI(const CelParams& p, uint64_t seed, float3 P, float3 N) noexcept
{
    ...
    for (int n = 0; n < kN; ++n)
    {
        ...
        const CelAtmo f = celSkyField(dir, p);
        float3 le = celSunDisc(dir, f.trans, p) + celSunAureole(dir, f.trans, p);
        if (dir.y > 0.0f)
        {
            le = le + f.sky;
        }
...
int main(int argc, char** argv)
{
    ...
    const CelParams p = ProjectZero::MakePanelParams(sun);
        ...
        for (uint32_t y = 0; y < kH; ++y)
        {
            for (uint32_t x = 0; x < kW; ++x)
            {
                ...
                else
                {
                    ...
                    const float3 hdr = celSkyFull(dir, p);
